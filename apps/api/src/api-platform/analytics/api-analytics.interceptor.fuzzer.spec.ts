import { ApiAnalyticsInterceptor } from './api-analytics.interceptor';
...
describe('ApiAnalyticsInterceptor (Direct Fuzzer Suite)', () => {
  let service: ApiAnalyticsInterceptor;

  beforeEach(() => {
    try {
      service = new ApiAnalyticsInterceptor(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ApiAnalyticsInterceptor(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ApiAnalyticsInterceptor.prototype).filter(m => m !== 'constructor');
