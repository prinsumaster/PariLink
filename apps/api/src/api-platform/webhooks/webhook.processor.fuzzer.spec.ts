...
describe('WebhookProcessor (Direct Fuzzer Suite)', () => {
  let service: WebhookProcessor;

  beforeEach(() => {
    try {
      service = new WebhookProcessor(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WebhookProcessor(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WebhookProcessor.prototype).filter(m => m !== 'constructor');
