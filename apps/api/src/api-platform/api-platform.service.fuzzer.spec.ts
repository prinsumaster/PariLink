...
describe('ApiPlatformService (Direct Fuzzer Suite)', () => {
  let service: ApiPlatformService;

  beforeEach(() => {
    try {
      service = new ApiPlatformService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ApiPlatformService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ApiPlatformService.prototype).filter(m => m !== 'constructor');
