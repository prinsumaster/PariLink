...
describe('WebhooksV2Controller (Direct Fuzzer Suite)', () => {
  let service: WebhooksV2Controller;

  beforeEach(() => {
    try {
      service = new WebhooksV2Controller(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WebhooksV2Controller(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WebhooksV2Controller.prototype).filter(m => m !== 'constructor');
