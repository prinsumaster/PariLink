...
describe('LoadsV2Controller (Direct Fuzzer Suite)', () => {
  let service: LoadsV2Controller;

  beforeEach(() => {
    try {
      service = new LoadsV2Controller(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LoadsV2Controller(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LoadsV2Controller.prototype).filter(m => m !== 'constructor');
