import { ApiV2AuthGuard } from './api-v2-auth.guard';
...
describe('ApiV2AuthGuard (Direct Fuzzer Suite)', () => {
  let service: ApiV2AuthGuard;

  beforeEach(() => {
    try {
      service = new ApiV2AuthGuard(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ApiV2AuthGuard(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ApiV2AuthGuard.prototype).filter(m => m !== 'constructor');
