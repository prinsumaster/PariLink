...
describe('BackupService (Direct Fuzzer Suite)', () => {
  let service: BackupService;

  beforeEach(() => {
    try {
      service = new BackupService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BackupService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BackupService.prototype).filter(m => m !== 'constructor');
  ...
  describe('triggerTenantBackup (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.triggerTenantBackup(...args as any);
        } catch (e) {
