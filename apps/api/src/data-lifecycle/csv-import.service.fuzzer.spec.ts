...
describe('CsvImportService (Direct Fuzzer Suite)', () => {
  let service: CsvImportService;

  beforeEach(() => {
    try {
      service = new CsvImportService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CsvImportService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CsvImportService.prototype).filter(m => m !== 'constructor');
  ...
  describe('importDriversTransactional (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.importDriversTransactional(...args as any);
        } catch (e) {
