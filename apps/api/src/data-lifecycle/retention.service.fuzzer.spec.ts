...
describe('RetentionService (Direct Fuzzer Suite)', () => {
  let service: RetentionService;

  beforeEach(() => {
    try {
      service = new RetentionService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RetentionService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RetentionService.prototype).filter(m => m !== 'constructor');
  ...
  describe('enforceRetentionPolicies (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.enforceRetentionPolicies(...args as any);
        } catch (e) {
