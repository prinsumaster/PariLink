...
describe('ApiAnalyticsService (Direct Fuzzer Suite)', () => {
  let service: ApiAnalyticsService;

  beforeEach(() => {
    try {
      service = new ApiAnalyticsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ApiAnalyticsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ApiAnalyticsService.prototype).filter(m => m !== 'constructor');
