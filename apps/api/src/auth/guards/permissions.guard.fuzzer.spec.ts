...
describe('PermissionsGuard (Direct Fuzzer Suite)', () => {
  let service: PermissionsGuard;

  beforeEach(() => {
    try {
      service = new PermissionsGuard(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PermissionsGuard(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PermissionsGuard.prototype).filter(m => m !== 'constructor');
