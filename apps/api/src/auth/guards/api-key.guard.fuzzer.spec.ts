...
describe('ApiKeyGuard (Direct Fuzzer Suite)', () => {
  let service: ApiKeyGuard;

  beforeEach(() => {
    try {
      service = new ApiKeyGuard(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ApiKeyGuard(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ApiKeyGuard.prototype).filter(m => m !== 'constructor');
