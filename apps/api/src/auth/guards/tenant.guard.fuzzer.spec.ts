...
describe('TenantGuard (Direct Fuzzer Suite)', () => {
  let service: TenantGuard;

  beforeEach(() => {
    try {
      service = new TenantGuard(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TenantGuard(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TenantGuard.prototype).filter(m => m !== 'constructor');
