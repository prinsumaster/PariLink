...
describe('JwtStrategy (Direct Fuzzer Suite)', () => {
  let service: JwtStrategy;

  beforeEach(() => {
    try {
      service = new JwtStrategy(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new JwtStrategy(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(JwtStrategy.prototype).filter(m => m !== 'constructor');
  ...
  describe('validate (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.validate(...args as any);
        } catch (e) {
