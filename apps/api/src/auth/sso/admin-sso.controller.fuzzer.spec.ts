...
describe('AdminSsoController (Direct Fuzzer Suite)', () => {
  let service: AdminSsoController;

  beforeEach(() => {
    try {
      service = new AdminSsoController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AdminSsoController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AdminSsoController.prototype).filter(m => m !== 'constructor');
