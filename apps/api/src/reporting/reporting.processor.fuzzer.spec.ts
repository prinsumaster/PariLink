...
describe('ReportingProcessor (Direct Fuzzer Suite)', () => {
  let service: ReportingProcessor;

  beforeEach(() => {
    try {
      service = new ReportingProcessor(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ReportingProcessor(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ReportingProcessor.prototype).filter(m => m !== 'constructor');
