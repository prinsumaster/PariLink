...
describe('CommentsController (Direct Fuzzer Suite)', () => {
  let service: CommentsController;

  beforeEach(() => {
    try {
      service = new CommentsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CommentsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CommentsController.prototype).filter(m => m !== 'constructor');
