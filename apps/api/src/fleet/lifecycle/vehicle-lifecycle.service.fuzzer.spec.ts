...
describe('VehicleLifecycleService (Direct Fuzzer Suite)', () => {
  let service: VehicleLifecycleService;

  beforeEach(() => {
    try {
      service = new VehicleLifecycleService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new VehicleLifecycleService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(VehicleLifecycleService.prototype).filter(m => m !== 'constructor');
