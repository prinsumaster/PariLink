...
describe('VehicleLifecycleController (Direct Fuzzer Suite)', () => {
  let service: VehicleLifecycleController;

  beforeEach(() => {
    try {
      service = new VehicleLifecycleController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new VehicleLifecycleController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(VehicleLifecycleController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getFleetStatus (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getFleetStatus(...args as any);
        } catch (e) {
  ...
  describe('onboardVehicle (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.onboardVehicle(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('getLocations (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getLocations(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
