...
describe('IoTController (Direct Fuzzer Suite)', () => {
  let service: IoTController;

  beforeEach(() => {
    try {
      service = new IoTController(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new IoTController(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(IoTController.prototype).filter(m => m !== 'constructor');
