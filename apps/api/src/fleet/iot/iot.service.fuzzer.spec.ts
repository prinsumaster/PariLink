...
describe('IoTService (Direct Fuzzer Suite)', () => {
  let service: IoTService;

  beforeEach(() => {
    try {
      service = new IoTService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new IoTService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(IoTService.prototype).filter(m => m !== 'constructor');
