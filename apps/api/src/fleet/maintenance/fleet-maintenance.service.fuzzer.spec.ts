...
describe('FleetMaintenanceService (Direct Fuzzer Suite)', () => {
  let service: FleetMaintenanceService;

  beforeEach(() => {
    try {
      service = new FleetMaintenanceService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FleetMaintenanceService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FleetMaintenanceService.prototype).filter(m => m !== 'constructor');
