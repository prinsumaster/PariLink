...
describe('LocalizationService (Direct Fuzzer Suite)', () => {
  let service: LocalizationService;

  beforeEach(() => {
    try {
      service = new LocalizationService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LocalizationService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LocalizationService.prototype).filter(m => m !== 'constructor');
