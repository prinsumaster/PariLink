...
describe('ExportsService (Direct Fuzzer Suite)', () => {
  let service: ExportsService;

  beforeEach(() => {
    try {
      service = new ExportsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ExportsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ExportsService.prototype).filter(m => m !== 'constructor');
  ...
  describe('generateExport (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.generateExport(...args as any);
        } catch (e) {
