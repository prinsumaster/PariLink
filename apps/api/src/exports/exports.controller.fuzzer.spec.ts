...
describe('ExportsController (Direct Fuzzer Suite)', () => {
  let service: ExportsController;

  beforeEach(() => {
    try {
      service = new ExportsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ExportsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ExportsController.prototype).filter(m => m !== 'constructor');
  ...
  describe('requestExport (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.requestExport(...args as any);
        } catch (e) {
  ...
  describe('getExports (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getExports(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
