...
describe('PdfGeneratorService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PdfGeneratorService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PdfGeneratorService.prototype).filter(m => m !== 'constructor');
  ...
  describe('generateBiltyPdf (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.generateBiltyPdf(...args as any);
        } catch (e) {
