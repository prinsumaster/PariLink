...
describe('BiltyController (Direct Fuzzer Suite)', () => {
  let service: BiltyController;

  beforeEach(() => {
    try {
      service = new BiltyController(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BiltyController(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BiltyController.prototype).filter(m => m !== 'constructor');
