import { WmsOrchestratorService } from './wms-orchestrator.service';
...
describe('WmsOrchestratorService (Direct Fuzzer Suite)', () => {
  let service: WmsOrchestratorService;

  beforeEach(() => {
    try {
      service = new WmsOrchestratorService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WmsOrchestratorService(null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WmsOrchestratorService.prototype).filter(m => m !== 'constructor');
  ...
  describe('handleWmsEvents (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.handleWmsEvents(...args as any);
        } catch (e) {
