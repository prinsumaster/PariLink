...
describe('WarehouseMasterService (Direct Fuzzer Suite)', () => {
  let service: WarehouseMasterService;

  beforeEach(() => {
    try {
      service = new WarehouseMasterService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WarehouseMasterService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WarehouseMasterService.prototype).filter(m => m !== 'constructor');
  ...
  describe('createZone (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.createZone(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('createBin (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.createBin(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
