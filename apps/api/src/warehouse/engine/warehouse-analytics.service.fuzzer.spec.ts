...
describe('WarehouseAnalyticsService (Direct Fuzzer Suite)', () => {
  let service: WarehouseAnalyticsService;

  beforeEach(() => {
    try {
      service = new WarehouseAnalyticsService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WarehouseAnalyticsService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WarehouseAnalyticsService.prototype).filter(m => m !== 'constructor');
  ...
  describe('getWarehouseAnalytics (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.getWarehouseAnalytics(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
