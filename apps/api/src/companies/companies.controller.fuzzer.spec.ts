...
describe('CompaniesController (Direct Fuzzer Suite)', () => {
  let service: CompaniesController;

  beforeEach(() => {
    try {
      service = new CompaniesController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CompaniesController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CompaniesController.prototype).filter(m => m !== 'constructor');
