...
describe('CompaniesService (Direct Fuzzer Suite)', () => {
  let service: CompaniesService;

  beforeEach(() => {
    try {
      service = new CompaniesService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CompaniesService(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CompaniesService.prototype).filter(m => m !== 'constructor');
