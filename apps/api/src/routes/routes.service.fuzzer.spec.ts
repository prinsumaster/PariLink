...
describe('RoutesService (Direct Fuzzer Suite)', () => {
  let service: RoutesService;

  beforeEach(() => {
    try {
      service = new RoutesService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RoutesService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RoutesService.prototype).filter(m => m !== 'constructor');
