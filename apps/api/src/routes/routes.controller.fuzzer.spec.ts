...
describe('RoutesController (Direct Fuzzer Suite)', () => {
  let service: RoutesController;

  beforeEach(() => {
    try {
      service = new RoutesController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RoutesController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RoutesController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getTollEstimate (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getTollEstimate(...args as any);
        } catch (e) {
