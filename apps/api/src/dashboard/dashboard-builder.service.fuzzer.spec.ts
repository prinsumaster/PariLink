...
describe('DashboardBuilderService (Direct Fuzzer Suite)', () => {
  let service: DashboardBuilderService;

  beforeEach(() => {
    try {
      service = new DashboardBuilderService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DashboardBuilderService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DashboardBuilderService.prototype).filter(m => m !== 'constructor');
