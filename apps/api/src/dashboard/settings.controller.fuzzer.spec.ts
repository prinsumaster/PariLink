...
describe('SettingsController (Direct Fuzzer Suite)', () => {
  let service: SettingsController;

  beforeEach(() => {
    try {
      service = new SettingsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SettingsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SettingsController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getOrganizationSettings (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getOrganizationSettings(...args as any);
        } catch (e) {
