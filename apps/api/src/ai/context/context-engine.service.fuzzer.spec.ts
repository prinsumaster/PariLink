...
describe('ContextEngineService (Direct Fuzzer Suite)', () => {
  let service: ContextEngineService;

  beforeEach(() => {
    try {
      service = new ContextEngineService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ContextEngineService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ContextEngineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('buildAiContext (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.buildAiContext(...args as any);
        } catch (e) {
