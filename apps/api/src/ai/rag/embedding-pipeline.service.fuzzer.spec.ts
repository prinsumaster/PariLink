...
describe('EmbeddingPipelineService (Direct Fuzzer Suite)', () => {
  let service: EmbeddingPipelineService;

  beforeEach(() => {
    try {
      service = new EmbeddingPipelineService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EmbeddingPipelineService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EmbeddingPipelineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('indexDocument (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.indexDocument(...args as any);
        } catch (e) {
