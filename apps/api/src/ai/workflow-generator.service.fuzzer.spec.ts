...
describe('WorkflowGeneratorService (Direct Fuzzer Suite)', () => {
  let service: WorkflowGeneratorService;

  beforeEach(() => {
    try {
      service = new WorkflowGeneratorService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WorkflowGeneratorService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WorkflowGeneratorService.prototype).filter(m => m !== 'constructor');
  ...
  describe('generateWorkflowGraph (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.generateWorkflowGraph(...args as any);
        } catch (e) {
