...
describe('AiCacheService (Direct Fuzzer Suite)', () => {
  let service: AiCacheService;

  beforeEach(() => {
    try {
      service = new AiCacheService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AiCacheService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AiCacheService.prototype).filter(m => m !== 'constructor');
  ...
  describe('getOrSetCache (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getOrSetCache(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
