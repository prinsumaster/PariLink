import { PromptProtectionService } from './prompt-protection.service';
...
describe('PromptProtectionService (Direct Fuzzer Suite)', () => {
  let service: PromptProtectionService;

  beforeEach(() => {
    try {
      service = new PromptProtectionService();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PromptProtectionService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PromptProtectionService.prototype).filter(m => m !== 'constructor');
