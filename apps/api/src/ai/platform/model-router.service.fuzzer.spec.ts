...
describe('ModelRouterService (Direct Fuzzer Suite)', () => {
  let service: ModelRouterService;

  beforeEach(() => {
    try {
      service = new ModelRouterService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ModelRouterService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ModelRouterService.prototype).filter(m => m !== 'constructor');
  ...
  describe('refreshModels (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.refreshModels(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('getBestModel (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getBestModel(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('getModelEntry (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getModelEntry(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
