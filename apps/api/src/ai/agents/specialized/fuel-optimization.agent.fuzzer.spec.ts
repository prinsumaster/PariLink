import { FuelOptimizationAgent } from './fuel-optimization.agent';
...
describe('FuelOptimizationAgent (Direct Fuzzer Suite)', () => {
  let service: FuelOptimizationAgent;

  beforeEach(() => {
    try {
      service = new FuelOptimizationAgent(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FuelOptimizationAgent(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FuelOptimizationAgent.prototype).filter(m => m !== 'constructor');
