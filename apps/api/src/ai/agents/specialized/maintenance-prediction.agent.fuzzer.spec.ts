import { MaintenancePredictionAgent } from './maintenance-prediction.agent';
...
describe('MaintenancePredictionAgent (Direct Fuzzer Suite)', () => {
  let service: MaintenancePredictionAgent;

  beforeEach(() => {
    try {
      service = new MaintenancePredictionAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new MaintenancePredictionAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(MaintenancePredictionAgent.prototype).filter(m => m !== 'constructor');
