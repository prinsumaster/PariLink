import { ShipmentDelayAgent } from './shipment-delay.agent';
...
describe('ShipmentDelayAgent (Direct Fuzzer Suite)', () => {
  let service: ShipmentDelayAgent;

  beforeEach(() => {
    try {
      service = new ShipmentDelayAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ShipmentDelayAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ShipmentDelayAgent.prototype).filter(m => m !== 'constructor');
