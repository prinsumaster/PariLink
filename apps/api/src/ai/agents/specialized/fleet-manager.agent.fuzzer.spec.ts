import { FleetManagerAgent } from './fleet-manager.agent';
...
describe('FleetManagerAgent (Direct Fuzzer Suite)', () => {
  let service: FleetManagerAgent;

  beforeEach(() => {
    try {
      service = new FleetManagerAgent(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FleetManagerAgent(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FleetManagerAgent.prototype).filter(m => m !== 'constructor');
