...
describe('ComplianceAgent (Direct Fuzzer Suite)', () => {
  let service: ComplianceAgent;

  beforeEach(() => {
    try {
      service = new ComplianceAgent(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ComplianceAgent(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ComplianceAgent.prototype).filter(m => m !== 'constructor');
