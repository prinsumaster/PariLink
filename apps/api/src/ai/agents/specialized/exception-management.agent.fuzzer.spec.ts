import { ExceptionManagementAgent } from './exception-management.agent';
...
describe('ExceptionManagementAgent (Direct Fuzzer Suite)', () => {
  let service: ExceptionManagementAgent;

  beforeEach(() => {
    try {
      service = new ExceptionManagementAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ExceptionManagementAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ExceptionManagementAgent.prototype).filter(m => m !== 'constructor');
