...
describe('WarehouseAgent (Direct Fuzzer Suite)', () => {
  let service: WarehouseAgent;

  beforeEach(() => {
    try {
      service = new WarehouseAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WarehouseAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WarehouseAgent.prototype).filter(m => m !== 'constructor');
