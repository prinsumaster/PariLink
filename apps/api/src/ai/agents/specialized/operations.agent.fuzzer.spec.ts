...
describe('OperationsAgent (Direct Fuzzer Suite)', () => {
  let service: OperationsAgent;

  beforeEach(() => {
    try {
      service = new OperationsAgent(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new OperationsAgent(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(OperationsAgent.prototype).filter(m => m !== 'constructor');
