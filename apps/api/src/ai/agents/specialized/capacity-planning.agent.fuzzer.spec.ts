...
describe('CapacityPlanningAgent (Direct Fuzzer Suite)', () => {
  let service: CapacityPlanningAgent;

  beforeEach(() => {
    try {
      service = new CapacityPlanningAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CapacityPlanningAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CapacityPlanningAgent.prototype).filter(m => m !== 'constructor');
