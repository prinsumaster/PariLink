import { CustomerSlaRiskAgent } from './customer-sla-risk.agent';
...
describe('CustomerSlaRiskAgent (Direct Fuzzer Suite)', () => {
  let service: CustomerSlaRiskAgent;

  beforeEach(() => {
    try {
      service = new CustomerSlaRiskAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CustomerSlaRiskAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CustomerSlaRiskAgent.prototype).filter(m => m !== 'constructor');
