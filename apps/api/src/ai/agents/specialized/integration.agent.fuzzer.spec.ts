...
describe('IntegrationAgent (Direct Fuzzer Suite)', () => {
  let service: IntegrationAgent;

  beforeEach(() => {
    try {
      service = new IntegrationAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new IntegrationAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(IntegrationAgent.prototype).filter(m => m !== 'constructor');
