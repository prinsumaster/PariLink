import { DriverSafetyAgent } from './driver-safety.agent';
...
describe('DriverSafetyAgent (Direct Fuzzer Suite)', () => {
  let service: DriverSafetyAgent;

  beforeEach(() => {
    try {
      service = new DriverSafetyAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverSafetyAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverSafetyAgent.prototype).filter(m => m !== 'constructor');
