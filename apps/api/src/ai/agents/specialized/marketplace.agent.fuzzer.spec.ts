...
describe('MarketplaceAgent (Direct Fuzzer Suite)', () => {
  let service: MarketplaceAgent;

  beforeEach(() => {
    try {
      service = new MarketplaceAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new MarketplaceAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(MarketplaceAgent.prototype).filter(m => m !== 'constructor');
