import { RevenueLeakageAgent } from './revenue-leakage.agent';
...
describe('RevenueLeakageAgent (Direct Fuzzer Suite)', () => {
  let service: RevenueLeakageAgent;

  beforeEach(() => {
    try {
      service = new RevenueLeakageAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RevenueLeakageAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RevenueLeakageAgent.prototype).filter(m => m !== 'constructor');
