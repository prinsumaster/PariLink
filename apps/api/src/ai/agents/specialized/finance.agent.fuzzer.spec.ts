...
describe('FinanceAgent (Direct Fuzzer Suite)', () => {
  let service: FinanceAgent;

  beforeEach(() => {
    try {
      service = new FinanceAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FinanceAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FinanceAgent.prototype).filter(m => m !== 'constructor');
