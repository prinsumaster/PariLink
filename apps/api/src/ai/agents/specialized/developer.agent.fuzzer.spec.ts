...
describe('DeveloperAgent (Direct Fuzzer Suite)', () => {
  let service: DeveloperAgent;

  beforeEach(() => {
    try {
      service = new DeveloperAgent(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DeveloperAgent(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DeveloperAgent.prototype).filter(m => m !== 'constructor');
