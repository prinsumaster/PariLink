...
describe('SupportAgent (Direct Fuzzer Suite)', () => {
  let service: SupportAgent;

  beforeEach(() => {
    try {
      service = new SupportAgent(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SupportAgent(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SupportAgent.prototype).filter(m => m !== 'constructor');
