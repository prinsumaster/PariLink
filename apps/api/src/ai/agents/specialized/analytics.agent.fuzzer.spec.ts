...
describe('AnalyticsAgent (Direct Fuzzer Suite)', () => {
  let service: AnalyticsAgent;

  beforeEach(() => {
    try {
      service = new AnalyticsAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AnalyticsAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AnalyticsAgent.prototype).filter(m => m !== 'constructor');
