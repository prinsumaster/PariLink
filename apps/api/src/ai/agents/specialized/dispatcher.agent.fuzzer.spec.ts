...
describe('DispatcherAgent (Direct Fuzzer Suite)', () => {
  let service: DispatcherAgent;

  beforeEach(() => {
    try {
      service = new DispatcherAgent(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DispatcherAgent(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DispatcherAgent.prototype).filter(m => m !== 'constructor');
