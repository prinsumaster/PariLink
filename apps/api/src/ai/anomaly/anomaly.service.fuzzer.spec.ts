...
describe('AnomalyDetectionService (Direct Fuzzer Suite)', () => {
  let service: AnomalyDetectionService;

  beforeEach(() => {
    try {
      service = new AnomalyDetectionService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AnomalyDetectionService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AnomalyDetectionService.prototype).filter(m => m !== 'constructor');
