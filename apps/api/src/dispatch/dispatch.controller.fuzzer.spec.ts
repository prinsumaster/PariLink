...
describe('DispatchController (Direct Fuzzer Suite)', () => {
  let service: DispatchController;

  beforeEach(() => {
    try {
      service = new DispatchController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DispatchController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DispatchController.prototype).filter(m => m !== 'constructor');
