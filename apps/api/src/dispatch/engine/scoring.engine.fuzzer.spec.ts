...
describe('ScoringEngine (Direct Fuzzer Suite)', () => {
  let service: ScoringEngine;

  beforeEach(() => {
    try {
      service = new ScoringEngine();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ScoringEngine();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ScoringEngine.prototype).filter(m => m !== 'constructor');
  ...
  describe('score (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.score(...args as any);
        } catch (e) {
