...
describe('PlanningService (Direct Fuzzer Suite)', () => {
  let service: PlanningService;

  beforeEach(() => {
    try {
      service = new PlanningService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PlanningService(null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PlanningService.prototype).filter(m => m !== 'constructor');
