...
describe('ExceptionService (Direct Fuzzer Suite)', () => {
  let service: ExceptionService;

  beforeEach(() => {
    try {
      service = new ExceptionService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ExceptionService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ExceptionService.prototype).filter(m => m !== 'constructor');
  ...
  describe('handleDriverRejection (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.handleDriverRejection(...args as any);
        } catch (e) {
