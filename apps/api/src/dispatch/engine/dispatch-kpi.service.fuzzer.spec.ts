...
describe('DispatchKpiService (Direct Fuzzer Suite)', () => {
  let service: DispatchKpiService;

  beforeEach(() => {
    try {
      service = new DispatchKpiService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DispatchKpiService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DispatchKpiService.prototype).filter(m => m !== 'constructor');
