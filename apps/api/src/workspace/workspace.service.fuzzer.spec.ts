...
describe('WorkspaceService (Direct Fuzzer Suite)', () => {
  let service: WorkspaceService;

  beforeEach(() => {
    try {
      service = new WorkspaceService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WorkspaceService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WorkspaceService.prototype).filter(m => m !== 'constructor');
