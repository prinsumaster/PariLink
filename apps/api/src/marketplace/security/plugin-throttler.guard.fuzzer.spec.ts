import { PluginApiThrottlerGuard } from './plugin-throttler.guard';
...
describe('PluginApiThrottlerGuard (Direct Fuzzer Suite)', () => {
  let service: PluginApiThrottlerGuard;

  beforeEach(() => {
    try {
      service = new PluginApiThrottlerGuard();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PluginApiThrottlerGuard();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PluginApiThrottlerGuard.prototype).filter(m => m !== 'constructor');
