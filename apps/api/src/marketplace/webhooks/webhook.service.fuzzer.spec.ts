...
describe('WebhookService (Direct Fuzzer Suite)', () => {
  ...
  beforeEach(() => {
    try {
      service = new WebhookService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WebhookService(null, null);
    } catch(e) {}
  ...
  describe('dispatchEvent (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.dispatchEvent(...args as any);
        } catch (e) {
