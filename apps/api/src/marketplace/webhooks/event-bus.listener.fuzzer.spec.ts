...
describe('EventBusListener (Direct Fuzzer Suite)', () => {
  let service: EventBusListener;

  beforeEach(() => {
    try {
      service = new EventBusListener(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EventBusListener(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EventBusListener.prototype).filter(m => m !== 'constructor');
