import { AppInstallationGuard } from './app-installation.guard';
...
describe('AppInstallationGuard (Direct Fuzzer Suite)', () => {
  let service: AppInstallationGuard;

  beforeEach(() => {
    try {
      service = new AppInstallationGuard(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AppInstallationGuard(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AppInstallationGuard.prototype).filter(m => m !== 'constructor');
