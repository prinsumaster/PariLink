...
describe('MarketplaceExtensionsController (Direct Fuzzer Suite)', () => {
  let service: MarketplaceExtensionsController;

  beforeEach(() => {
    try {
      service = new MarketplaceExtensionsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new MarketplaceExtensionsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(MarketplaceExtensionsController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getExtensionsForSlot (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getExtensionsForSlot(...args as any);
        } catch (e) {
