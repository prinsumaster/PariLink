...
describe('TelemetryIngressController (Direct Fuzzer Suite)', () => {
  let service: TelemetryIngressController;

  beforeEach(() => {
    try {
      service = new TelemetryIngressController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TelemetryIngressController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TelemetryIngressController.prototype).filter(m => m !== 'constructor');
