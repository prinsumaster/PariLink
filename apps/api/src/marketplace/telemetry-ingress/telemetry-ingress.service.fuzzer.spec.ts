...
describe('TelemetryIngressService (Direct Fuzzer Suite)', () => {
  let service: TelemetryIngressService;

  beforeEach(() => {
    try {
      service = new TelemetryIngressService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TelemetryIngressService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TelemetryIngressService.prototype).filter(m => m !== 'constructor');
  ...
  describe('processIncomingTelemetry (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.processIncomingTelemetry(...args as any);
        } catch (e) {
