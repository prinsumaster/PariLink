import { TelemetryIngressWorker } from './telemetry-queue.design';
...
describe('TelemetryIngressWorker (Direct Fuzzer Suite)', () => {
  let service: TelemetryIngressWorker;

  beforeEach(() => {
    try {
      service = new TelemetryIngressWorker(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TelemetryIngressWorker(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TelemetryIngressWorker.prototype).filter(m => m !== 'constructor');
