...
describe('LedgerService (Direct Fuzzer Suite)', () => {
  let service: LedgerService;

  beforeEach(() => {
    try {
      service = new LedgerService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LedgerService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LedgerService.prototype).filter(m => m !== 'constructor');
