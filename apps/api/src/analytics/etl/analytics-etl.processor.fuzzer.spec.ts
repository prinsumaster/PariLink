import { AnalyticsETLProcessor } from './analytics-etl.processor';
...
describe('AnalyticsETLProcessor (Direct Fuzzer Suite)', () => {
  let service: AnalyticsETLProcessor;

  beforeEach(() => {
    try {
      service = new AnalyticsETLProcessor(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AnalyticsETLProcessor(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AnalyticsETLProcessor.prototype).filter(m => m !== 'constructor');
