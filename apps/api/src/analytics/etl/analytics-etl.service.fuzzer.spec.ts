...
describe('AnalyticsETLService (Direct Fuzzer Suite)', () => {
  let service: AnalyticsETLService;

  beforeEach(() => {
    try {
      service = new AnalyticsETLService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AnalyticsETLService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AnalyticsETLService.prototype).filter(m => m !== 'constructor');
  ...
  describe('runDailyAggregation (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.runDailyAggregation(...args as any);
        } catch (e) {
