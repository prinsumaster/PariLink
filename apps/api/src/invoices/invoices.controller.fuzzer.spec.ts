...
describe('InvoicesController (Direct Fuzzer Suite)', () => {
  let service: InvoicesController;

  beforeEach(() => {
    try {
      service = new InvoicesController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new InvoicesController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(InvoicesController.prototype).filter(m => m !== 'constructor');
