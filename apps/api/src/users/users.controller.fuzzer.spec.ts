...
describe('UsersController (Direct Fuzzer Suite)', () => {
  let service: UsersController;

  beforeEach(() => {
    try {
      service = new UsersController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new UsersController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(UsersController.prototype).filter(m => m !== 'constructor');
