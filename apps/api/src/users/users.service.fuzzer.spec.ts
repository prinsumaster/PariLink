...
describe('UsersService (Direct Fuzzer Suite)', () => {
  let service: UsersService;

  beforeEach(() => {
    try {
      service = new UsersService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new UsersService(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(UsersService.prototype).filter(m => m !== 'constructor');
