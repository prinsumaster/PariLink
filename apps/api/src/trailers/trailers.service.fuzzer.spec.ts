...
describe('TrailersService (Direct Fuzzer Suite)', () => {
  let service: TrailersService;

  beforeEach(() => {
    try {
      service = new TrailersService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TrailersService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TrailersService.prototype).filter(m => m !== 'constructor');
