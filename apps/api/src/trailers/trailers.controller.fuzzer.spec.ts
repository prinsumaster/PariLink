...
describe('TrailersController (Direct Fuzzer Suite)', () => {
  let service: TrailersController;

  beforeEach(() => {
    try {
      service = new TrailersController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TrailersController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TrailersController.prototype).filter(m => m !== 'constructor');
