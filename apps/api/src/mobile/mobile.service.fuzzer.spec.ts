...
describe('MobileService (Direct Fuzzer Suite)', () => {
  let service: MobileService;

  beforeEach(() => {
    try {
      service = new MobileService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new MobileService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(MobileService.prototype).filter(m => m !== 'constructor');
