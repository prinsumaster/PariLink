...
describe('DeveloperController (Direct Fuzzer Suite)', () => {
  let service: DeveloperController;

  beforeEach(() => {
    try {
      service = new DeveloperController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DeveloperController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DeveloperController.prototype).filter(m => m !== 'constructor');
  ...
  describe('rotateSecret (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.rotateSecret(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
