...
describe('FuelEntriesService (Direct Fuzzer Suite)', () => {
  let service: FuelEntriesService;

  beforeEach(() => {
    try {
      service = new FuelEntriesService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FuelEntriesService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FuelEntriesService.prototype).filter(m => m !== 'constructor');
  ...
  describe('getFuelByTrip (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getFuelByTrip(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
