...
describe('TripDesksController (Direct Fuzzer Suite)', () => {
  let service: TripDesksController;

  beforeEach(() => {
    try {
      service = new TripDesksController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TripDesksController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TripDesksController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getTripDesks (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getTripDesks(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
