...
describe('TripsController (Direct Fuzzer Suite)', () => {
  let service: TripsController;

  beforeEach(() => {
    try {
      service = new TripsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TripsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TripsController.prototype).filter(m => m !== 'constructor');
