...
describe('DriverSettlementService (Direct Fuzzer Suite)', () => {
  let service: DriverSettlementService;

  beforeEach(() => {
    try {
      service = new DriverSettlementService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverSettlementService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverSettlementService.prototype).filter(m => m !== 'constructor');
  ...
  describe('calculateSettlement (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.calculateSettlement(...args as any);
        } catch (e) {
