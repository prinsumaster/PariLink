...
describe('LoadingEventsController (Direct Fuzzer Suite)', () => {
  let service: LoadingEventsController;

  beforeEach(() => {
    try {
      service = new LoadingEventsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LoadingEventsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LoadingEventsController.prototype).filter(m => m !== 'constructor');
