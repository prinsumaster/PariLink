...
describe('IdempotencyInterceptor (Direct Fuzzer Suite)', () => {
  let service: IdempotencyInterceptor;

  beforeEach(() => {
    try {
      service = new IdempotencyInterceptor(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new IdempotencyInterceptor(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(IdempotencyInterceptor.prototype).filter(m => m !== 'constructor');
