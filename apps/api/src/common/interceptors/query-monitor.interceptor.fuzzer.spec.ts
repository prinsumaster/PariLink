import { QueryMonitorInterceptor } from './query-monitor.interceptor';
...
describe('QueryMonitorInterceptor (Direct Fuzzer Suite)', () => {
  let service: QueryMonitorInterceptor;

  beforeEach(() => {
    try {
      service = new QueryMonitorInterceptor();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new QueryMonitorInterceptor();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(QueryMonitorInterceptor.prototype).filter(m => m !== 'constructor');
