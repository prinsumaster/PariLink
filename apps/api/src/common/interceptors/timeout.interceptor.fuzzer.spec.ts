...
describe('TimeoutInterceptor (Direct Fuzzer Suite)', () => {
  let service: TimeoutInterceptor;

  beforeEach(() => {
    try {
      service = new TimeoutInterceptor();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TimeoutInterceptor();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TimeoutInterceptor.prototype).filter(m => m !== 'constructor');
