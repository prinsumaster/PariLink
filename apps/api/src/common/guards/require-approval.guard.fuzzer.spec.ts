import { RequireApprovalGuard } from './require-approval.guard';
...
describe('RequireApprovalGuard (Direct Fuzzer Suite)', () => {
  let service: RequireApprovalGuard;

  beforeEach(() => {
    try {
      service = new RequireApprovalGuard(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RequireApprovalGuard(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RequireApprovalGuard.prototype).filter(m => m !== 'constructor');
