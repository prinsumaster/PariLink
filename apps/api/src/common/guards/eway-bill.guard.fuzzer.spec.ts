import { EWayBillGuard } from './eway-bill.guard';
...
describe('EWayBillGuard (Direct Fuzzer Suite)', () => {
  let service: EWayBillGuard;

  beforeEach(() => {
    try {
      service = new EWayBillGuard(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EWayBillGuard(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EWayBillGuard.prototype).filter(m => m !== 'constructor');
