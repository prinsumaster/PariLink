...
describe('DlpGuard (Direct Fuzzer Suite)', () => {
  let service: DlpGuard;

  beforeEach(() => {
    try {
      service = new DlpGuard(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DlpGuard(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DlpGuard.prototype).filter(m => m !== 'constructor');
