import { HeuristicOptimizerService } from './heuristic-optimizer.service';
...
describe('HeuristicOptimizerService (Direct Fuzzer Suite)', () => {
  let service: HeuristicOptimizerService;

  beforeEach(() => {
    try {
      service = new HeuristicOptimizerService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new HeuristicOptimizerService(null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(HeuristicOptimizerService.prototype).filter(m => m !== 'constructor');
