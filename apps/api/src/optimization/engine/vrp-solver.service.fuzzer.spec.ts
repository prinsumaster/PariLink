...
describe('VrpSolverService (Direct Fuzzer Suite)', () => {
  let service: VrpSolverService;

  beforeEach(() => {
    try {
      service = new VrpSolverService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new VrpSolverService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(VrpSolverService.prototype).filter(m => m !== 'constructor');
  ...
  describe('solve (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.solve(...args as any);
        } catch (e) {
