...
describe('SimulationService (Direct Fuzzer Suite)', () => {
  let service: SimulationService;

  beforeEach(() => {
    try {
      service = new SimulationService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SimulationService(null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SimulationService.prototype).filter(m => m !== 'constructor');
