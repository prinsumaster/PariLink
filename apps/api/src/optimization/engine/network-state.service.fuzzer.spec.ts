...
describe('NetworkStateService (Direct Fuzzer Suite)', () => {
  let service: NetworkStateService;

  beforeEach(() => {
    try {
      service = new NetworkStateService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new NetworkStateService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(NetworkStateService.prototype).filter(m => m !== 'constructor');
  ...
  describe('buildOptimizationContext (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.buildOptimizationContext(...args as any);
        } catch (e) {
