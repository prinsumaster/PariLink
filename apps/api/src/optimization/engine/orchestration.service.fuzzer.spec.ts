...
describe('OrchestrationService (Direct Fuzzer Suite)', () => {
  let service: OrchestrationService;

  beforeEach(() => {
    try {
      service = new OrchestrationService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new OrchestrationService(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(OrchestrationService.prototype).filter(m => m !== 'constructor');
