...
describe('ScoringService (Direct Fuzzer Suite)', () => {
  let service: ScoringService;

  beforeEach(() => {
    try {
      service = new ScoringService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ScoringService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ScoringService.prototype).filter(m => m !== 'constructor');
  ...
  describe('getWeights (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getWeights(...args as any);
        } catch (e) {
  ...
  describe('calculateScore (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.calculateScore(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
