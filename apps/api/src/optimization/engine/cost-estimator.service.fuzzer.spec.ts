...
describe('CostEstimatorService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CostEstimatorService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CostEstimatorService.prototype).filter(m => m !== 'constructor');
  ...
  describe('estimate (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.estimate(...args as any);
        } catch (e) {
