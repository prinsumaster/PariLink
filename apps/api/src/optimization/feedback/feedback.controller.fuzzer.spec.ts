...
describe('FeedbackController (Direct Fuzzer Suite)', () => {
  let service: FeedbackController;

  beforeEach(() => {
    try {
      service = new FeedbackController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FeedbackController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FeedbackController.prototype).filter(m => m !== 'constructor');
  ...
  describe('recordFeedback (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.recordFeedback(...args as any);
        } catch (e) {
