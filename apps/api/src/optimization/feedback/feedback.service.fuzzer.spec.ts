...
describe('FeedbackService (Direct Fuzzer Suite)', () => {
  let service: FeedbackService;

  beforeEach(() => {
    try {
      service = new FeedbackService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FeedbackService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FeedbackService.prototype).filter(m => m !== 'constructor');
