...
describe('SupportController (Direct Fuzzer Suite)', () => {
  let service: SupportController;

  beforeEach(() => {
    try {
      service = new SupportController();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SupportController();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SupportController.prototype).filter(m => m !== 'constructor');
  ...
  describe('createTicket (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.createTicket(...args as any);
        } catch (e) {
