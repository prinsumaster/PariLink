...
describe('SlaTrackerService (Direct Fuzzer Suite)', () => {
  let service: SlaTrackerService;

  beforeEach(() => {
    try {
      service = new SlaTrackerService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SlaTrackerService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SlaTrackerService.prototype).filter(m => m !== 'constructor');
  ...
  describe('evaluateCustomerSLA (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.evaluateCustomerSLA(...args as any);
        } catch (e) {
