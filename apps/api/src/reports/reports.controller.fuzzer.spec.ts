...
describe('ReportsController (Direct Fuzzer Suite)', () => {
  let service: ReportsController;

  beforeEach(() => {
    try {
      service = new ReportsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ReportsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ReportsController.prototype).filter(m => m !== 'constructor');
