...
describe('TenantProvisioningService (Direct Fuzzer Suite)', () => {
  let service: TenantProvisioningService;

  beforeEach(() => {
    try {
      service = new TenantProvisioningService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TenantProvisioningService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TenantProvisioningService.prototype).filter(m => m !== 'constructor');
