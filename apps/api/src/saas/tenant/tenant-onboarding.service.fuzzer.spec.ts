...
describe('TenantOnboardingService (Direct Fuzzer Suite)', () => {
  let service: TenantOnboardingService;

  beforeEach(() => {
    try {
      service = new TenantOnboardingService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TenantOnboardingService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TenantOnboardingService.prototype).filter(m => m !== 'constructor');
  ...
  describe('uploadLogo (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.uploadLogo(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
