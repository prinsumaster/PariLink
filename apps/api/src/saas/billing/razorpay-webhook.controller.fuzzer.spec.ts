...
describe('RazorpayWebhookController (Direct Fuzzer Suite)', () => {
  let service: RazorpayWebhookController;

  beforeEach(() => {
    try {
      service = new RazorpayWebhookController(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RazorpayWebhookController(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RazorpayWebhookController.prototype).filter(m => m !== 'constructor');
