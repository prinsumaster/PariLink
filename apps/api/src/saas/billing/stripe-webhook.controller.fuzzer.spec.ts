...
describe('StripeWebhookController (Direct Fuzzer Suite)', () => {
  let service: StripeWebhookController;

  beforeEach(() => {
    try {
      service = new StripeWebhookController(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new StripeWebhookController(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(StripeWebhookController.prototype).filter(m => m !== 'constructor');
