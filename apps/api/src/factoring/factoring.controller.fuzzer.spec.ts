...
describe('FactoringController (Direct Fuzzer Suite)', () => {
  let service: FactoringController;

  beforeEach(() => {
    try {
      service = new FactoringController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FactoringController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FactoringController.prototype).filter(m => m !== 'constructor');
  ...
  describe('submit (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.submit(...args as any);
        } catch (e) {
