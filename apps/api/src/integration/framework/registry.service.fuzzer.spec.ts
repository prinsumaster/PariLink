import { ConnectorRegistryService } from './registry.service';
...
describe('ConnectorRegistryService (Direct Fuzzer Suite)', () => {
  let service: ConnectorRegistryService;

  beforeEach(() => {
    try {
      service = new ConnectorRegistryService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ConnectorRegistryService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ConnectorRegistryService.prototype).filter(m => m !== 'constructor');
  ...
  describe('listAvailableConnectors (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [],
      ...
      for (const args of argSets) {
        try {
          await service.listAvailableConnectors(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
