...
describe('ImportExportController (Direct Fuzzer Suite)', () => {
  let service: ImportExportController;

  beforeEach(() => {
    try {
      service = new ImportExportController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ImportExportController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ImportExportController.prototype).filter(m => m !== 'constructor');
