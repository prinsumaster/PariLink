...
describe('EnterpriseEventBusService (Direct Fuzzer Suite)', () => {
  let service: EnterpriseEventBusService;

  beforeEach(() => {
    try {
      service = new EnterpriseEventBusService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EnterpriseEventBusService(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EnterpriseEventBusService.prototype).filter(m => m !== 'constructor');
