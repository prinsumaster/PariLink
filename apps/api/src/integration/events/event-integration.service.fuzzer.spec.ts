...
describe('EventIntegrationService (Direct Fuzzer Suite)', () => {
  let service: EventIntegrationService;

  beforeEach(() => {
    try {
      service = new EventIntegrationService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EventIntegrationService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EventIntegrationService.prototype).filter(m => m !== 'constructor');
