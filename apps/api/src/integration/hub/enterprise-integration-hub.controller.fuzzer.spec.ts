...
describe('EnterpriseIntegrationHubController (Direct Fuzzer Suite)', () => {
  let service: EnterpriseIntegrationHubController;

  beforeEach(() => {
    try {
      service = new EnterpriseIntegrationHubController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EnterpriseIntegrationHubController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EnterpriseIntegrationHubController.prototype).filter(m => m !== 'constructor');
