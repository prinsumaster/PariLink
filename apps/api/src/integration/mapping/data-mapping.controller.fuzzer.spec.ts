...
describe('DataMappingController (Direct Fuzzer Suite)', () => {
  let service: DataMappingController;

  beforeEach(() => {
    try {
      service = new DataMappingController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DataMappingController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DataMappingController.prototype).filter(m => m !== 'constructor');
