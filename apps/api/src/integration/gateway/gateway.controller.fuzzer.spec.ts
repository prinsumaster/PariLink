import { IntegrationGatewayController } from './gateway.controller';
...
describe('IntegrationGatewayController (Direct Fuzzer Suite)', () => {
  let service: IntegrationGatewayController;

  beforeEach(() => {
    try {
      service = new IntegrationGatewayController(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new IntegrationGatewayController(null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(IntegrationGatewayController.prototype).filter(m => m !== 'constructor');
  ...
  describe('handleIncomingWebhook (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.handleIncomingWebhook(...args as any);
        } catch (e) {
