import { IntegrationObservabilityService } from './observability.service';
...
describe('IntegrationObservabilityService (Direct Fuzzer Suite)', () => {
  let service: IntegrationObservabilityService;

  beforeEach(() => {
    try {
      service = new IntegrationObservabilityService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new IntegrationObservabilityService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(IntegrationObservabilityService.prototype).filter(m => m !== 'constructor');
