import { XeroConnector } from './erp.connectors';
...
describe('XeroConnector (Direct Fuzzer Suite)', () => {
  let service: XeroConnector;

  beforeEach(() => {
    try {
      service = new XeroConnector();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new XeroConnector();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(XeroConnector.prototype).filter(m => m !== 'constructor');
