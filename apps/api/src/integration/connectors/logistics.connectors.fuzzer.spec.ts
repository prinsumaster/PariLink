import { PaymentGatewayConnector } from './logistics.connectors';
...
describe('PaymentGatewayConnector (Direct Fuzzer Suite)', () => {
  let service: PaymentGatewayConnector;

  beforeEach(() => {
    try {
      service = new PaymentGatewayConnector();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PaymentGatewayConnector();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PaymentGatewayConnector.prototype).filter(m => m !== 'constructor');
