...
describe('QuickBooksConnector (Direct Fuzzer Suite)', () => {
  let service: QuickBooksConnector;

  beforeEach(() => {
    try {
      service = new QuickBooksConnector();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new QuickBooksConnector();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(QuickBooksConnector.prototype).filter(m => m !== 'constructor');
