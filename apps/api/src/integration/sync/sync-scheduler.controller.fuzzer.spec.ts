...
describe('SyncSchedulerController (Direct Fuzzer Suite)', () => {
  let service: SyncSchedulerController;

  beforeEach(() => {
    try {
      service = new SyncSchedulerController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SyncSchedulerController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SyncSchedulerController.prototype).filter(m => m !== 'constructor');
