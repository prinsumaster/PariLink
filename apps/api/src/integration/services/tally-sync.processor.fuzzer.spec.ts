import { TallySyncProcessor } from './tally-sync.processor';
...
describe('TallySyncProcessor (Direct Fuzzer Suite)', () => {
  let service: TallySyncProcessor;

  beforeEach(() => {
    try {
      service = new TallySyncProcessor(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TallySyncProcessor(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TallySyncProcessor.prototype).filter(m => m !== 'constructor');
  ...
  describe('exportInvoicesToTally (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.exportInvoicesToTally(...args as any);
        } catch (e) {
