import { FastagWalletController } from './fastag-wallet.controller';
...
describe('FastagWalletController (Direct Fuzzer Suite)', () => {
  let service: FastagWalletController;

  beforeEach(() => {
    try {
      service = new FastagWalletController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FastagWalletController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FastagWalletController.prototype).filter(m => m !== 'constructor');
