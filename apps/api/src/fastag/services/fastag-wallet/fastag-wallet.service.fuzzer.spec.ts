import { FastagWalletService } from './fastag-wallet.service';
...
describe('FastagWalletService (Direct Fuzzer Suite)', () => {
  let service: FastagWalletService;

  beforeEach(() => {
    try {
      service = new FastagWalletService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FastagWalletService(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FastagWalletService.prototype).filter(m => m !== 'constructor');
