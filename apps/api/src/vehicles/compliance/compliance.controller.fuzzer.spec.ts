...
describe('ComplianceController (Direct Fuzzer Suite)', () => {
  let service: ComplianceController;

  beforeEach(() => {
    try {
      service = new ComplianceController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ComplianceController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ComplianceController.prototype).filter(m => m !== 'constructor');
