...
describe('FuelController (Direct Fuzzer Suite)', () => {
  let service: FuelController;

  beforeEach(() => {
    try {
      service = new FuelController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FuelController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FuelController.prototype).filter(m => m !== 'constructor');
