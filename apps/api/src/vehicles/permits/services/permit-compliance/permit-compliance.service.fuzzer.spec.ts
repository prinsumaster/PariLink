...
describe('PermitComplianceService (Direct Fuzzer Suite)', () => {
  let service: PermitComplianceService;

  beforeEach(() => {
    try {
      service = new PermitComplianceService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PermitComplianceService(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PermitComplianceService.prototype).filter(m => m !== 'constructor');
