...
describe('PermitController (Direct Fuzzer Suite)', () => {
  let service: PermitController;

  beforeEach(() => {
    try {
      service = new PermitController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PermitController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PermitController.prototype).filter(m => m !== 'constructor');
