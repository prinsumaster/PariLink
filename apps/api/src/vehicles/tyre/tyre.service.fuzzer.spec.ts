...
describe('TyreService (Direct Fuzzer Suite)', () => {
  let service: TyreService;

  beforeEach(() => {
    try {
      service = new TyreService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TyreService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TyreService.prototype).filter(m => m !== 'constructor');
