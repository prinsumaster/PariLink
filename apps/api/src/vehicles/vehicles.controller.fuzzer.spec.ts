...
describe('VehiclesController (Direct Fuzzer Suite)', () => {
  let service: VehiclesController;

  beforeEach(() => {
    try {
      service = new VehiclesController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new VehiclesController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(VehiclesController.prototype).filter(m => m !== 'constructor');
