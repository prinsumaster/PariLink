...
describe('FleetOrchestratorService (Direct Fuzzer Suite)', () => {
  let service: FleetOrchestratorService;

  beforeEach(() => {
    try {
      service = new FleetOrchestratorService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FleetOrchestratorService(null, null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FleetOrchestratorService.prototype).filter(m => m !== 'constructor');
  ...
  describe('handleFleetEvents (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.handleFleetEvents(...args as any);
        } catch (e) {
