...
describe('ComplianceEngine (Direct Fuzzer Suite)', () => {
  let service: ComplianceEngine;

  beforeEach(() => {
    try {
      service = new ComplianceEngine(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ComplianceEngine(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ComplianceEngine.prototype).filter(m => m !== 'constructor');
  ...
  describe('evaluateDocumentCompliance (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.evaluateDocumentCompliance(...args as any);
        } catch (e) {
