...
describe('ProfitabilityService (Direct Fuzzer Suite)', () => {
  let service: ProfitabilityService;

  beforeEach(() => {
    try {
      service = new ProfitabilityService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ProfitabilityService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ProfitabilityService.prototype).filter(m => m !== 'constructor');
