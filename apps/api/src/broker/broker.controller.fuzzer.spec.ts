...
describe('BrokerController (Direct Fuzzer Suite)', () => {
  let service: BrokerController;

  beforeEach(() => {
    try {
      service = new BrokerController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BrokerController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BrokerController.prototype).filter(m => m !== 'constructor');
