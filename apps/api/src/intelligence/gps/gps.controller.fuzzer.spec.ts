...
describe('GpsController (Direct Fuzzer Suite)', () => {
  let service: GpsController;

  beforeEach(() => {
    try {
      service = new GpsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new GpsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(GpsController.prototype).filter(m => m !== 'constructor');
  ...
  describe('ingestPing (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.ingestPing(...args as any);
        } catch (e) {
