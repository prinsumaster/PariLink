...
describe('GpsService (Direct Fuzzer Suite)', () => {
  let service: GpsService;

  beforeEach(() => {
    try {
      service = new GpsService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new GpsService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(GpsService.prototype).filter(m => m !== 'constructor');
