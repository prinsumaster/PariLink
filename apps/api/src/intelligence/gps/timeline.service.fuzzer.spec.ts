...
describe('TimelineService (Direct Fuzzer Suite)', () => {
  let service: TimelineService;

  beforeEach(() => {
    try {
      service = new TimelineService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TimelineService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TimelineService.prototype).filter(m => m !== 'constructor');
