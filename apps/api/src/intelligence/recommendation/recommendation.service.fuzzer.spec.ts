...
describe('RecommendationService (Direct Fuzzer Suite)', () => {
  let service: RecommendationService;

  beforeEach(() => {
    try {
      service = new RecommendationService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RecommendationService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RecommendationService.prototype).filter(m => m !== 'constructor');
