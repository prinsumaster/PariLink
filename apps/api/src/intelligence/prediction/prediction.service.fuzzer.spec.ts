...
describe('PredictionService (Direct Fuzzer Suite)', () => {
  let service: PredictionService;

  beforeEach(() => {
    try {
      service = new PredictionService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PredictionService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PredictionService.prototype).filter(m => m !== 'constructor');
