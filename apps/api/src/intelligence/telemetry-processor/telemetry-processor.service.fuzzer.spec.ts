...
describe('TelemetryProcessorService (Direct Fuzzer Suite)', () => {
  let service: TelemetryProcessorService;

  beforeEach(() => {
    try {
      service = new TelemetryProcessorService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TelemetryProcessorService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TelemetryProcessorService.prototype).filter(m => m !== 'constructor');
  ...
  describe('handleGpsPing (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.handleGpsPing(...args as any);
        } catch (e) {
