...
describe('OccService (Direct Fuzzer Suite)', () => {
  let service: OccService;

  beforeEach(() => {
    try {
      service = new OccService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new OccService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(OccService.prototype).filter(m => m !== 'constructor');
  ...
  describe('getFleetTree (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getFleetTree(...args as any);
        } catch (e) {
  ...
  describe('getTimeline (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getTimeline(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
