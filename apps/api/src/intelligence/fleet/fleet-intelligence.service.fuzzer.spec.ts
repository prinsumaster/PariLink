...
describe('FleetIntelligenceService (Direct Fuzzer Suite)', () => {
  let service: FleetIntelligenceService;

  beforeEach(() => {
    try {
      service = new FleetIntelligenceService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FleetIntelligenceService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FleetIntelligenceService.prototype).filter(m => m !== 'constructor');
