...
describe('FleetIntelligenceController (Direct Fuzzer Suite)', () => {
  let service: FleetIntelligenceController;

  beforeEach(() => {
    try {
      service = new FleetIntelligenceController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FleetIntelligenceController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FleetIntelligenceController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getFleetKpis (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getFleetKpis(...args as any);
        } catch (e) {
