...
describe('DispatchAiService (Direct Fuzzer Suite)', () => {
  let service: DispatchAiService;

  beforeEach(() => {
    try {
      service = new DispatchAiService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DispatchAiService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DispatchAiService.prototype).filter(m => m !== 'constructor');
