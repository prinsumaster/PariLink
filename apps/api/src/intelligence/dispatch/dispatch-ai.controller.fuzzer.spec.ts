...
describe('DispatchAiController (Direct Fuzzer Suite)', () => {
  let service: DispatchAiController;

  beforeEach(() => {
    try {
      service = new DispatchAiController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DispatchAiController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DispatchAiController.prototype).filter(m => m !== 'constructor');
  ...
  describe('requestRecommendation (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.requestRecommendation(...args as any);
        } catch (e) {
