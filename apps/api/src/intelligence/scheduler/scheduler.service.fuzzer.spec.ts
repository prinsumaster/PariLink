...
describe('AiSchedulerService (Direct Fuzzer Suite)', () => {
  let service: AiSchedulerService;

  beforeEach(() => {
    try {
      service = new AiSchedulerService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AiSchedulerService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AiSchedulerService.prototype).filter(m => m !== 'constructor');
  ...
  describe('handleNightlyRiskRecalculation (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.handleNightlyRiskRecalculation(...args as any);
        } catch (e) {
