...
describe('DriverScoringService (Direct Fuzzer Suite)', () => {
  let service: DriverScoringService;

  beforeEach(() => {
    try {
      service = new DriverScoringService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverScoringService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverScoringService.prototype).filter(m => m !== 'constructor');
  ...
  describe('handleAlert (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.handleAlert(...args as any);
        } catch (e) {
