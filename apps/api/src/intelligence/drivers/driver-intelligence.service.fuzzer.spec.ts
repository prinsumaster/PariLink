...
describe('DriverIntelligenceService (Direct Fuzzer Suite)', () => {
  let service: DriverIntelligenceService;

  beforeEach(() => {
    try {
      service = new DriverIntelligenceService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverIntelligenceService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverIntelligenceService.prototype).filter(m => m !== 'constructor');
