...
describe('DriverIntelligenceController (Direct Fuzzer Suite)', () => {
  let service: DriverIntelligenceController;

  beforeEach(() => {
    try {
      service = new DriverIntelligenceController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverIntelligenceController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverIntelligenceController.prototype).filter(m => m !== 'constructor');
