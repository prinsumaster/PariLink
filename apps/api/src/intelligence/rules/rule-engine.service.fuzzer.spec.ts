...
describe('RuleEngineService (Direct Fuzzer Suite)', () => {
  let service: RuleEngineService;

  beforeEach(() => {
    try {
      service = new RuleEngineService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RuleEngineService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RuleEngineService.prototype).filter(m => m !== 'constructor');
