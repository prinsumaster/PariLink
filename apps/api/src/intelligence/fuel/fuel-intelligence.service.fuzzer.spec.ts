...
describe('FuelIntelligenceService (Direct Fuzzer Suite)', () => {
  let service: FuelIntelligenceService;

  beforeEach(() => {
    try {
      service = new FuelIntelligenceService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FuelIntelligenceService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FuelIntelligenceService.prototype).filter(m => m !== 'constructor');
