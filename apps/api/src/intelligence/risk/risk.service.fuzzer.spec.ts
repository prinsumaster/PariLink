...
describe('RiskService (Direct Fuzzer Suite)', () => {
  let service: RiskService;

  beforeEach(() => {
    try {
      service = new RiskService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RiskService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RiskService.prototype).filter(m => m !== 'constructor');
  ...
  describe('assessTripRisk (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.assessTripRisk(...args as any);
        } catch (e) {
