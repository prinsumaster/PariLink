...
describe('GeofenceService (Direct Fuzzer Suite)', () => {
  let service: GeofenceService;

  beforeEach(() => {
    try {
      service = new GeofenceService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new GeofenceService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(GeofenceService.prototype).filter(m => m !== 'constructor');
