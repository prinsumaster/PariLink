...
describe('EtaIntelligenceService (Direct Fuzzer Suite)', () => {
  let service: EtaIntelligenceService;

  beforeEach(() => {
    try {
      service = new EtaIntelligenceService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EtaIntelligenceService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EtaIntelligenceService.prototype).filter(m => m !== 'constructor');
