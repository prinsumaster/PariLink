...
describe('EtaIntelligenceController (Direct Fuzzer Suite)', () => {
  let service: EtaIntelligenceController;

  beforeEach(() => {
    try {
      service = new EtaIntelligenceController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EtaIntelligenceController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EtaIntelligenceController.prototype).filter(m => m !== 'constructor');
  ...
  describe('predictEta (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.predictEta(...args as any);
        } catch (e) {
