...
describe('AlertEngineService (Direct Fuzzer Suite)', () => {
  let service: AlertEngineService;

  beforeEach(() => {
    try {
      service = new AlertEngineService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AlertEngineService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AlertEngineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('evaluateGpsPing (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.evaluateGpsPing(...args as any);
        } catch (e) {
