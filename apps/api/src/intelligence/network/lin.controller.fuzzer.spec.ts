...
describe('LinController (Direct Fuzzer Suite)', () => {
  let service: LinController;

  beforeEach(() => {
    try {
      service = new LinController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LinController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LinController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getBenchmarks (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getBenchmarks(...args as any);
        } catch (e) {
