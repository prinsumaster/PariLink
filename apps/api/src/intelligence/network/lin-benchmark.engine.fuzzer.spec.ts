...
describe('LinBenchmarkEngine (Direct Fuzzer Suite)', () => {
  let service: LinBenchmarkEngine;

  beforeEach(() => {
    try {
      service = new LinBenchmarkEngine(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LinBenchmarkEngine(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LinBenchmarkEngine.prototype).filter(m => m !== 'constructor');
  ...
  describe('computeNetworkBenchmarks (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.computeNetworkBenchmarks(...args as any);
        } catch (e) {
