...
describe('DriversController (Direct Fuzzer Suite)', () => {
  let service: DriversController;

  beforeEach(() => {
    try {
      service = new DriversController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriversController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriversController.prototype).filter(m => m !== 'constructor');
