...
describe('AttendanceTrackingService (Direct Fuzzer Suite)', () => {
  let service: AttendanceTrackingService;

  beforeEach(() => {
    try {
      service = new AttendanceTrackingService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AttendanceTrackingService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AttendanceTrackingService.prototype).filter(m => m !== 'constructor');
