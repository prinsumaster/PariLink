...
describe('AttendanceController (Direct Fuzzer Suite)', () => {
  let service: AttendanceController;

  beforeEach(() => {
    try {
      service = new AttendanceController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AttendanceController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AttendanceController.prototype).filter(m => m !== 'constructor');
