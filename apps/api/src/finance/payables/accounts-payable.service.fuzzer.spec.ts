...
describe('AccountsPayableService (Direct Fuzzer Suite)', () => {
  let service: AccountsPayableService;

  beforeEach(() => {
    try {
      service = new AccountsPayableService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AccountsPayableService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AccountsPayableService.prototype).filter(m => m !== 'constructor');
