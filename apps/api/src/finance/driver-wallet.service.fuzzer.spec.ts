...
describe('DriverWalletService (Direct Fuzzer Suite)', () => {
  let service: DriverWalletService;

  beforeEach(() => {
    try {
      service = new DriverWalletService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverWalletService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverWalletService.prototype).filter(m => m !== 'constructor');
