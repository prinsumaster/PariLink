...
describe('FastagService (Direct Fuzzer Suite)', () => {
  let service: FastagService;

  beforeEach(() => {
    try {
      service = new FastagService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FastagService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FastagService.prototype).filter(m => m !== 'constructor');
