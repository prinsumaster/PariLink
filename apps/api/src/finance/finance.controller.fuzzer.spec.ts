...
describe('FinanceController (Direct Fuzzer Suite)', () => {
  let service: FinanceController;

  beforeEach(() => {
    try {
      service = new FinanceController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FinanceController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FinanceController.prototype).filter(m => m !== 'constructor');
