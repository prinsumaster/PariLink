...
describe('SettlementEngine (Direct Fuzzer Suite)', () => {
  let service: SettlementEngine;

  beforeEach(() => {
    try {
      service = new SettlementEngine(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SettlementEngine(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SettlementEngine.prototype).filter(m => m !== 'constructor');
