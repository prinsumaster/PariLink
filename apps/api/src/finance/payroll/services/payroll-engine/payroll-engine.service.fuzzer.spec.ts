...
describe('PayrollEngineService (Direct Fuzzer Suite)', () => {
  let service: PayrollEngineService;

  beforeEach(() => {
    try {
      service = new PayrollEngineService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PayrollEngineService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PayrollEngineService.prototype).filter(m => m !== 'constructor');
