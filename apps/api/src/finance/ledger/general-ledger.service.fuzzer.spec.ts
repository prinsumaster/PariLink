...
describe('GeneralLedgerService (Direct Fuzzer Suite)', () => {
  let service: GeneralLedgerService;

  beforeEach(() => {
    try {
      service = new GeneralLedgerService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new GeneralLedgerService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(GeneralLedgerService.prototype).filter(m => m !== 'constructor');
