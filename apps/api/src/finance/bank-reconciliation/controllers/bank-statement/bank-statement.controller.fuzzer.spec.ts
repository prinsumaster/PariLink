...
describe('BankStatementController (Direct Fuzzer Suite)', () => {
  let service: BankStatementController;

  beforeEach(() => {
    try {
      service = new BankStatementController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BankStatementController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BankStatementController.prototype).filter(m => m !== 'constructor');
