...
describe('BankSyncService (Direct Fuzzer Suite)', () => {
  let service: BankSyncService;

  beforeEach(() => {
    try {
      service = new BankSyncService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BankSyncService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BankSyncService.prototype).filter(m => m !== 'constructor');
