...
describe('InvoicingController (Direct Fuzzer Suite)', () => {
  let service: InvoicingController;

  beforeEach(() => {
    try {
      service = new InvoicingController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new InvoicingController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(InvoicingController.prototype).filter(m => m !== 'constructor');
