...
describe('InvoicingService (Direct Fuzzer Suite)', () => {
  let service: InvoicingService;

  beforeEach(() => {
    try {
      service = new InvoicingService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new InvoicingService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(InvoicingService.prototype).filter(m => m !== 'constructor');
