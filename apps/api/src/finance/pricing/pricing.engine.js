...
var PricingEngine = function () {
    ...
    var _classThis;
    var PricingEngine = _classThis = /** @class */ (function () {
        function PricingEngine_1(prisma) {
            this.prisma = prisma;
         ...
         */
        PricingEngine_1.prototype.calculatePrice = function (ctx) {
            return __awaiter(this, void 0, void 0, function () {
                var baseFreight, accessorialCharges, fuelSurcharge, fscMultiplier, overage, billableHours, detentionAmount, stopAmount, totalAccessorials, totalAmount;
                return __generator(this, function (_a) {
                    this.logger.log("Evaluating Pricing for Customer ".concat(ctx.customerId));
                    baseFreight = 0;
                    accessorialCharges = [];
                    ...
                    fuelSurcharge = 0;
                    if (ctx.currentFuelPrice &&
                        ctx.baseFuelPrice &&
                        ctx.currentFuelPrice > ctx.baseFuelPrice) {
                        fscMultiplier = 0.05;
                        overage = ctx.currentFuelPrice - ctx.baseFuelPrice;
                        fuelSurcharge = baseFreight * (overage * fscMultiplier);
                    }
                    // Detention Charges
                    if (ctx.detentionHours && ctx.detentionHours > 2) {
                        billableHours = ctx.detentionHours - 2;
                        detentionAmount = billableHours * 50;
                        accessorialCharges.push({
                            type: 'DETENTION',
                            amount: detentionAmount,
                            description: "".concat(billableHours, " hours detention"),
                        });
                    }
                    // Multi-stop Charges
                    if (ctx.stops && ctx.stops > 1) {
                        stopAmount = (ctx.stops - 1) * 75;
                        accessorialCharges.push({
                            type: 'EXTRA_STOP',
                            amount: stopAmount,
                            description: "".concat(ctx.stops - 1, " extra stops"),
                        });
                    }
                    // Hazmat
                    if (ctx.isHazmat) {
                    ...
                    totalAccessorials = accessorialCharges.reduce(function (acc, c) { return acc + c.amount; }, 0);
                    totalAmount = baseFreight + fuelSurcharge + totalAccessorials;
                    return [2 /*return*/, {
