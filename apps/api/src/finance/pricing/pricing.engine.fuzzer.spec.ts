...
describe('PricingEngine (Direct Fuzzer Suite)', () => {
  let service: PricingEngine;

  beforeEach(() => {
    try {
      service = new PricingEngine(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PricingEngine(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PricingEngine.prototype).filter(m => m !== 'constructor');
  ...
  describe('calculatePrice (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.calculatePrice(...args as any);
        } catch (e) {
