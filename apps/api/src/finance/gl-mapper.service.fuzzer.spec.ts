...
describe('GlMapperService (Direct Fuzzer Suite)', () => {
  let service: GlMapperService;

  beforeEach(() => {
    try {
      service = new GlMapperService();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new GlMapperService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(GlMapperService.prototype).filter(m => m !== 'constructor');
  ...
  describe('mapToGl (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.mapToGl(...args as any);
        } catch (e) {
