...
describe('SimulatorController (Direct Fuzzer Suite)', () => {
  let service: SimulatorController;

  beforeEach(() => {
    try {
      service = new SimulatorController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SimulatorController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SimulatorController.prototype).filter(m => m !== 'constructor');
  ...
  describe('startSimulation (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.startSimulation(...args as any);
        } catch (e) {
