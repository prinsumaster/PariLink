...
describe('SimulatorService (Direct Fuzzer Suite)', () => {
  let service: SimulatorService;

  beforeEach(() => {
    try {
      service = new SimulatorService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SimulatorService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SimulatorService.prototype).filter(m => m !== 'constructor');
