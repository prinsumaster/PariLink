...
describe('JobsService (Direct Fuzzer Suite)', () => {
  let service: JobsService;

  beforeEach(() => {
    try {
      service = new JobsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new JobsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(JobsService.prototype).filter(m => m !== 'constructor');
