...
describe('SandboxService (Direct Fuzzer Suite)', () => {
  let service: SandboxService;

  beforeEach(() => {
    try {
      service = new SandboxService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SandboxService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SandboxService.prototype).filter(m => m !== 'constructor');
