...
describe('SandboxController (Direct Fuzzer Suite)', () => {
  let service: SandboxController;

  beforeEach(() => {
    try {
      service = new SandboxController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SandboxController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SandboxController.prototype).filter(m => m !== 'constructor');
  ...
  describe('generateMockData (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.generateMockData(...args as any);
        } catch (e) {
