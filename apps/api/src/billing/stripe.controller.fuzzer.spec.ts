...
describe('StripeController (Direct Fuzzer Suite)', () => {
  let service: StripeController;

  beforeEach(() => {
    try {
      service = new StripeController(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new StripeController(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(StripeController.prototype).filter(m => m !== 'constructor');
