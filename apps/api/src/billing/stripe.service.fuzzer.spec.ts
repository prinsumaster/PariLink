...
describe('StripeBillingService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new StripeBillingService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(StripeBillingService.prototype).filter(m => m !== 'constructor');
