...
describe('BillingController (Direct Fuzzer Suite)', () => {
  let service: BillingController;

  beforeEach(() => {
    try {
      service = new BillingController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BillingController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BillingController.prototype).filter(m => m !== 'constructor');
