import { LicenseCapacityGuard } from './license-capacity.guard';
...
describe('LicenseCapacityGuard (Direct Fuzzer Suite)', () => {
  let service: LicenseCapacityGuard;

  beforeEach(() => {
    try {
      service = new LicenseCapacityGuard(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LicenseCapacityGuard(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LicenseCapacityGuard.prototype).filter(m => m !== 'constructor');
  ...
  describe('canActivate (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.canActivate(...args as any);
        } catch (e) {
