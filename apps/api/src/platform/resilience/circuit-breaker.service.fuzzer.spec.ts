...
describe('CircuitBreakerService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CircuitBreakerService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CircuitBreakerService.prototype).filter(m => m !== 'constructor');
  ...
  describe('configure (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.configure(...args as any);
        } catch (e) {
  ...
  describe('execute (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.execute(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('getStatus (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getStatus(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
