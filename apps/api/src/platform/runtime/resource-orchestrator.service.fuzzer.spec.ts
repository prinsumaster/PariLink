...
describe('ResourceOrchestratorService (Direct Fuzzer Suite)', () => {
  let service: ResourceOrchestratorService;

  beforeEach(() => {
    try {
      service = new ResourceOrchestratorService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ResourceOrchestratorService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ResourceOrchestratorService.prototype).filter(m => m !== 'constructor');
  ...
  describe('allocateResource (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.allocateResource(...args as any);
        } catch (e) {
