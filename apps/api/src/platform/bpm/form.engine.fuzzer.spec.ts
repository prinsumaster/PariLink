...
describe('FormEngine (Direct Fuzzer Suite)', () => {
  let service: FormEngine;

  beforeEach(() => {
    try {
      service = new FormEngine(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FormEngine(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FormEngine.prototype).filter(m => m !== 'constructor');
  ...
  describe('submitForm (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.submitForm(...args as any);
        } catch (e) {
