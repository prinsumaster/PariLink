...
describe('ProcessEngine (Direct Fuzzer Suite)', () => {
  let service: ProcessEngine;

  beforeEach(() => {
    try {
      service = new ProcessEngine(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ProcessEngine(null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ProcessEngine.prototype).filter(m => m !== 'constructor');
  ...
  describe('startProcess (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.startProcess(...args as any);
        } catch (e) {
  ...
  describe('resumeProcess (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.resumeProcess(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
