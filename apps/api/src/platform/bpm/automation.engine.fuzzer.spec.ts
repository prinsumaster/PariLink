...
describe('AutomationEngine (Direct Fuzzer Suite)', () => {
  let service: AutomationEngine;

  beforeEach(() => {
    try {
      service = new AutomationEngine(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AutomationEngine(null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AutomationEngine.prototype).filter(m => m !== 'constructor');
  ...
  describe('executeSystemTask (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.executeSystemTask(...args as any);
        } catch (e) {
