import { IamPolicyEngineService } from './iam-policy-engine.service';
...
describe('IamPolicyEngineService (Direct Fuzzer Suite)', () => {
  let service: IamPolicyEngineService;

  beforeEach(() => {
    try {
      service = new IamPolicyEngineService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new IamPolicyEngineService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(IamPolicyEngineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('authorize (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.authorize(...args as any);
        } catch (e) {
  ...
  describe('hasPermission (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.hasPermission(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('invalidateRoleCache (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.invalidateRoleCache(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
