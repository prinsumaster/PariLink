...
describe('LicenseService (Direct Fuzzer Suite)', () => {
  let service: LicenseService;

  beforeEach(() => {
    try {
      service = new LicenseService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LicenseService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LicenseService.prototype).filter(m => m !== 'constructor');
  ...
  describe('getLicense (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getLicense(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('requireModule (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.requireModule(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('checkLimit (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.checkLimit(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
