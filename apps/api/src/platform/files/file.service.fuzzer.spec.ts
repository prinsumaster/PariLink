...
describe('FilePlatformService (Direct Fuzzer Suite)', () => {
  let service: FilePlatformService;

  beforeEach(() => {
    try {
      service = new FilePlatformService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FilePlatformService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FilePlatformService.prototype).filter(m => m !== 'constructor');
  ...
  describe('processUpload (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.processUpload(...args as any);
        } catch (e) {
