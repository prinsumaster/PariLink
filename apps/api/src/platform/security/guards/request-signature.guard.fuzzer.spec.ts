...
describe('RequestSignatureGuard (Direct Fuzzer Suite)', () => {
  let service: RequestSignatureGuard;

  beforeEach(() => {
    try {
      service = new RequestSignatureGuard(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RequestSignatureGuard(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RequestSignatureGuard.prototype).filter(m => m !== 'constructor');
