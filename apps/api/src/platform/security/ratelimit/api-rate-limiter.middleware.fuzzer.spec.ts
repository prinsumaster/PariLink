...
describe('ApiRateLimiterMiddleware (Direct Fuzzer Suite)', () => {
  let service: ApiRateLimiterMiddleware;

  beforeEach(() => {
    try {
      service = new ApiRateLimiterMiddleware(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ApiRateLimiterMiddleware(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ApiRateLimiterMiddleware.prototype).filter(m => m !== 'constructor');
