...
describe('BruteForceProtectionService (Direct Fuzzer Suite)', () => {
  let service: BruteForceProtectionService;

  beforeEach(() => {
    try {
      service = new BruteForceProtectionService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BruteForceProtectionService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BruteForceProtectionService.prototype).filter(m => m !== 'constructor');
  ...
  describe('onModuleDestroy (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.onModuleDestroy(...args as any);
        } catch (e) {
  ...
  describe('checkLoginAttempt (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.checkLoginAttempt(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('recordFailedAttempt (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.recordFailedAttempt(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('recordSuccessfulLogin (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.recordSuccessfulLogin(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('adminUnlock (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.adminUnlock(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('isAccountLocked (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.isAccountLocked(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
