import { ObservabilityInterceptor } from './logging.interceptor';
...
describe('ObservabilityInterceptor (Direct Fuzzer Suite)', () => {
  let service: ObservabilityInterceptor;

  beforeEach(() => {
    try {
      service = new ObservabilityInterceptor(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ObservabilityInterceptor(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ObservabilityInterceptor.prototype).filter(m => m !== 'constructor');
  ...
  describe('intercept (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.intercept(...args as any);
        } catch (e) {
