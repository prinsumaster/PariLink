...
describe('EventService (Direct Fuzzer Suite)', () => {
  let service: EventService;

  beforeEach(() => {
    try {
      service = new EventService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EventService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EventService.prototype).filter(m => m !== 'constructor');
  ...
  describe('publish (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.publish(...args as any);
        } catch (e) {
  ...
  describe('publishAsync (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.publishAsync(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('publishOutbox (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.publishOutbox(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
