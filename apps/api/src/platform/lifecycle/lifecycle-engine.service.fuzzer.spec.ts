import { LifecycleEngineService } from './lifecycle-engine.service';
...
describe('LifecycleEngineService (Direct Fuzzer Suite)', () => {
  let service: LifecycleEngineService;

  beforeEach(() => {
    try {
      service = new LifecycleEngineService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LifecycleEngineService(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LifecycleEngineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('transitionState (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.transitionState(...args as any);
        } catch (e) {
