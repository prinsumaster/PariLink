...
describe('DataQualityEngineService (Direct Fuzzer Suite)', () => {
  let service: DataQualityEngineService;

  beforeEach(() => {
    try {
      service = new DataQualityEngineService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DataQualityEngineService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DataQualityEngineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('reevaluateTenant (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.reevaluateTenant(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
