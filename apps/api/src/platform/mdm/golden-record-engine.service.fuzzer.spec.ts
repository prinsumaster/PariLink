...
describe('GoldenRecordEngineService (Direct Fuzzer Suite)', () => {
  let service: GoldenRecordEngineService;

  beforeEach(() => {
    try {
      service = new GoldenRecordEngineService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new GoldenRecordEngineService(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(GoldenRecordEngineService.prototype).filter(m => m !== 'constructor');
