...
describe('ReferenceDataService (Direct Fuzzer Suite)', () => {
  let service: ReferenceDataService;

  beforeEach(() => {
    try {
      service = new ReferenceDataService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ReferenceDataService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ReferenceDataService.prototype).filter(m => m !== 'constructor');
  ...
  describe('getReferenceRecord (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.getReferenceRecord(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
