...
describe('MdmSearchService (Direct Fuzzer Suite)', () => {
  let service: MdmSearchService;

  beforeEach(() => {
    try {
      service = new MdmSearchService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new MdmSearchService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(MdmSearchService.prototype).filter(m => m !== 'constructor');
  ...
  describe('semanticSearch (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.semanticSearch(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
