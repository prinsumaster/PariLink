import { TwinSyncService } from './twin-sync.service';
...
describe('TwinSyncService (Direct Fuzzer Suite)', () => {
  let service: TwinSyncService;

  beforeEach(() => {
    try {
      service = new TwinSyncService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TwinSyncService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TwinSyncService.prototype).filter(m => m !== 'constructor');
