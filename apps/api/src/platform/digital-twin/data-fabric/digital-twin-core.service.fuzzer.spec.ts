...
describe('DigitalTwinCoreService (Direct Fuzzer Suite)', () => {
  let service: DigitalTwinCoreService;

  beforeEach(() => {
    try {
      service = new DigitalTwinCoreService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DigitalTwinCoreService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DigitalTwinCoreService.prototype).filter(m => m !== 'constructor');
