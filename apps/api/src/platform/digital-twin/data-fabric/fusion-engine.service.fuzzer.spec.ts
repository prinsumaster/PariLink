...
describe('FusionEngineService (Direct Fuzzer Suite)', () => {
  let service: FusionEngineService;

  beforeEach(() => {
    try {
      service = new FusionEngineService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FusionEngineService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FusionEngineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('processTelemetry (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.processTelemetry(...args as any);
        } catch (e) {
