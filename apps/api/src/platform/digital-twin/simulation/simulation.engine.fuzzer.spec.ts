...
describe('SimulationEngine (Direct Fuzzer Suite)', () => {
  let service: SimulationEngine;

  beforeEach(() => {
    try {
      service = new SimulationEngine(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SimulationEngine(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SimulationEngine.prototype).filter(m => m !== 'constructor');
  ...
  describe('runSimulation (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.runSimulation(...args as any);
        } catch (e) {
