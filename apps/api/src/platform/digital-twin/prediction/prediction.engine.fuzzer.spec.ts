...
describe('PredictionEngine (Direct Fuzzer Suite)', () => {
  let service: PredictionEngine;

  beforeEach(() => {
    try {
      service = new PredictionEngine(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PredictionEngine(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PredictionEngine.prototype).filter(m => m !== 'constructor');
  ...
  describe('predictState (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.predictState(...args as any);
        } catch (e) {
