...
describe('CacheManagerService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CacheManagerService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CacheManagerService.prototype).filter(m => m !== 'constructor');
  ...
  describe('invalidateByTag (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.invalidateByTag(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('invalidateTenant (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.invalidateTenant(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('generateTenantKey (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
      ...
      for (const args of argSets) {
        try {
          await service.generateTenantKey(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('stats (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.stats(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
