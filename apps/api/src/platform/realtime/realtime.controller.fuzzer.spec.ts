...
describe('RealtimeController (Direct Fuzzer Suite)', () => {
  let service: RealtimeController;

  beforeEach(() => {
    try {
      service = new RealtimeController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RealtimeController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RealtimeController.prototype).filter(m => m !== 'constructor');
  ...
  describe('stream (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.stream(...args as any);
        } catch (e) {
