...
describe('PluginRegistry (Direct Fuzzer Suite)', () => {
  let service: PluginRegistry;

  beforeEach(() => {
    try {
      service = new PluginRegistry(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PluginRegistry(null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PluginRegistry.prototype).filter(m => m !== 'constructor');
  ...
  describe('activateTenantPlugins (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.activateTenantPlugins(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
