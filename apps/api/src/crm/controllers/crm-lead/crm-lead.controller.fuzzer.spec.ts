...
describe('CrmLeadController (Direct Fuzzer Suite)', () => {
  let service: CrmLeadController;

  beforeEach(() => {
    try {
      service = new CrmLeadController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CrmLeadController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CrmLeadController.prototype).filter(m => m !== 'constructor');
