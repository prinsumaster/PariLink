...
describe('CrmLeadService (Direct Fuzzer Suite)', () => {
  let service: CrmLeadService;

  beforeEach(() => {
    try {
      service = new CrmLeadService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CrmLeadService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CrmLeadService.prototype).filter(m => m !== 'constructor');
