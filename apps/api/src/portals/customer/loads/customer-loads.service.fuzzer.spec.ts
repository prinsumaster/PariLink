...
describe('CustomerLoadsService (Direct Fuzzer Suite)', () => {
  let service: CustomerLoadsService;

  beforeEach(() => {
    try {
      service = new CustomerLoadsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CustomerLoadsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CustomerLoadsService.prototype).filter(m => m !== 'constructor');
