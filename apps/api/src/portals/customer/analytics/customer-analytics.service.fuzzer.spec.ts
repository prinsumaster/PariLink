...
describe('CustomerAnalyticsService (Direct Fuzzer Suite)', () => {
  let service: CustomerAnalyticsService;

  beforeEach(() => {
    try {
      service = new CustomerAnalyticsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CustomerAnalyticsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CustomerAnalyticsService.prototype).filter(m => m !== 'constructor');
