...
describe('CustomerAnalyticsController (Direct Fuzzer Suite)', () => {
  let service: CustomerAnalyticsController;

  beforeEach(() => {
    try {
      service = new CustomerAnalyticsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CustomerAnalyticsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CustomerAnalyticsController.prototype).filter(m => m !== 'constructor');
