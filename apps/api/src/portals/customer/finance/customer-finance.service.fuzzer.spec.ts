...
describe('CustomerFinanceService (Direct Fuzzer Suite)', () => {
  let service: CustomerFinanceService;

  beforeEach(() => {
    try {
      service = new CustomerFinanceService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CustomerFinanceService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CustomerFinanceService.prototype).filter(m => m !== 'constructor');
