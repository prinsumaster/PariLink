...
describe('PortalsService (Direct Fuzzer Suite)', () => {
  let service: PortalsService;

  beforeEach(() => {
    try {
      service = new PortalsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PortalsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PortalsService.prototype).filter(m => m !== 'constructor');
