...
describe('DriverExpensesController (Direct Fuzzer Suite)', () => {
  let service: DriverExpensesController;

  beforeEach(() => {
    try {
      service = new DriverExpensesController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverExpensesController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverExpensesController.prototype).filter(m => m !== 'constructor');
