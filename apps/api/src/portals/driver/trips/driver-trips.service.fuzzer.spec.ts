...
describe('DriverTripsService (Direct Fuzzer Suite)', () => {
  let service: DriverTripsService;

  beforeEach(() => {
    try {
      service = new DriverTripsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverTripsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverTripsService.prototype).filter(m => m !== 'constructor');
