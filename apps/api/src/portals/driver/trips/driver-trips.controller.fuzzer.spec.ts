...
describe('DriverTripsController (Direct Fuzzer Suite)', () => {
  let service: DriverTripsController;

  beforeEach(() => {
    try {
      service = new DriverTripsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverTripsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverTripsController.prototype).filter(m => m !== 'constructor');
