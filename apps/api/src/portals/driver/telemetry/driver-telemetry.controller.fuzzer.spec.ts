import { DriverTelemetryController } from './driver-telemetry.controller';
...
describe('DriverTelemetryController (Direct Fuzzer Suite)', () => {
  let service: DriverTelemetryController;

  beforeEach(() => {
    try {
      service = new DriverTelemetryController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverTelemetryController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverTelemetryController.prototype).filter(m => m !== 'constructor');
  ...
  describe('logLocation (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.logLocation(...args as any);
        } catch (e) {
