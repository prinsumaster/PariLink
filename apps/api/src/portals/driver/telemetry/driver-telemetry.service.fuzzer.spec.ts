...
describe('DriverTelemetryService (Direct Fuzzer Suite)', () => {
  let service: DriverTelemetryService;

  beforeEach(() => {
    try {
      service = new DriverTelemetryService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverTelemetryService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverTelemetryService.prototype).filter(m => m !== 'constructor');
