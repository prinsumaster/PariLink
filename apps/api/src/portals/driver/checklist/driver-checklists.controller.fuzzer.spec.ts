import { DriverChecklistsController } from './driver-checklists.controller';
...
describe('DriverChecklistsController (Direct Fuzzer Suite)', () => {
  let service: DriverChecklistsController;

  beforeEach(() => {
    try {
      service = new DriverChecklistsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverChecklistsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverChecklistsController.prototype).filter(m => m !== 'constructor');
  ...
  describe('submitChecklist (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.submitChecklist(...args as any);
        } catch (e) {
