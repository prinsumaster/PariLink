...
describe('DriverChecklistsService (Direct Fuzzer Suite)', () => {
  let service: DriverChecklistsService;

  beforeEach(() => {
    try {
      service = new DriverChecklistsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DriverChecklistsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DriverChecklistsService.prototype).filter(m => m !== 'constructor');
