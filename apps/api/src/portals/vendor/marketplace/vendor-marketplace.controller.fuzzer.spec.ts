...
describe('VendorMarketplaceController (Direct Fuzzer Suite)', () => {
  let service: VendorMarketplaceController;

  beforeEach(() => {
    try {
      service = new VendorMarketplaceController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new VendorMarketplaceController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(VendorMarketplaceController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getOpenTenders (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getOpenTenders(...args as any);
        } catch (e) {
