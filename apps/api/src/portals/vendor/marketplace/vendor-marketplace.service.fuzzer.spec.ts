...
describe('VendorMarketplaceService (Direct Fuzzer Suite)', () => {
  let service: VendorMarketplaceService;

  beforeEach(() => {
    try {
      service = new VendorMarketplaceService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new VendorMarketplaceService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(VendorMarketplaceService.prototype).filter(m => m !== 'constructor');
