...
describe('VendorSettlementsService (Direct Fuzzer Suite)', () => {
  let service: VendorSettlementsService;

  beforeEach(() => {
    try {
      service = new VendorSettlementsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new VendorSettlementsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(VendorSettlementsService.prototype).filter(m => m !== 'constructor');
