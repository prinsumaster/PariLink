...
describe('RolesController (Direct Fuzzer Suite)', () => {
  let service: RolesController;

  beforeEach(() => {
    try {
      service = new RolesController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RolesController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RolesController.prototype).filter(m => m !== 'constructor');
