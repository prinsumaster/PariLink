...
describe('RolesService (Direct Fuzzer Suite)', () => {
  let service: RolesService;

  beforeEach(() => {
    try {
      service = new RolesService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new RolesService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(RolesService.prototype).filter(m => m !== 'constructor');
