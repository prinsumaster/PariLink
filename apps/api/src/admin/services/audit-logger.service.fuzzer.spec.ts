...
describe('AuditLoggerService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AuditLoggerService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AuditLoggerService.prototype).filter(m => m !== 'constructor');
  ...
  describe('logAction (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.logAction(...args as any);
        } catch (e) {
