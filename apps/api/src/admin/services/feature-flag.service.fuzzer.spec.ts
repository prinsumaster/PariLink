...
describe('FeatureFlagService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new FeatureFlagService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(FeatureFlagService.prototype).filter(m => m !== 'constructor');
  ...
  describe('isFeatureEnabled (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.isFeatureEnabled(...args as any);
        } catch (e) {
  ...
  describe('getTenantFlags (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.getTenantFlags(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
