import { DashboardAdminController } from './enterprise-admin.controller';
...
describe('DashboardAdminController (Direct Fuzzer Suite)', () => {
  let service: DashboardAdminController;

  beforeEach(() => {
    try {
      service = new DashboardAdminController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DashboardAdminController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DashboardAdminController.prototype).filter(m => m !== 'constructor');
