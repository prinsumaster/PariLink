...
describe('EdiService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EdiService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EdiService.prototype).filter(m => m !== 'constructor');
  ...
  describe('parseX12 (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.parseX12(...args as any);
        } catch (e) {
  ...
  describe('generateX12 (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.generateX12(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
