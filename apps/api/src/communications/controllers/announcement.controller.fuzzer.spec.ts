...
describe('AnnouncementController (Direct Fuzzer Suite)', () => {
  let service: AnnouncementController;

  beforeEach(() => {
    try {
      service = new AnnouncementController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AnnouncementController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AnnouncementController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getAnnouncements (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getAnnouncements(...args as any);
        } catch (e) {
  ...
  describe('createAnnouncement (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.createAnnouncement(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
