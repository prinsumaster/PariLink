...
describe('InboxController (Direct Fuzzer Suite)', () => {
  let service: InboxController;

  beforeEach(() => {
    try {
      service = new InboxController(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new InboxController(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(InboxController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getThreads (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getThreads(...args as any);
        } catch (e) {
