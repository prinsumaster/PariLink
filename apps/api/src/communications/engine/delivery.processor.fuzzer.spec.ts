...
describe('DeliveryProcessor (Direct Fuzzer Suite)', () => {
  let service: DeliveryProcessor;

  beforeEach(() => {
    try {
      service = new DeliveryProcessor(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DeliveryProcessor(null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DeliveryProcessor.prototype).filter(m => m !== 'constructor');
