...
describe('TemplateService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TemplateService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TemplateService.prototype).filter(m => m !== 'constructor');
  ...
  describe('render (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.render(...args as any);
        } catch (e) {
