...
describe('SmsProvider (Direct Fuzzer Suite)', () => {
  let service: SmsProvider;

  beforeEach(() => {
    try {
      service = new SmsProvider();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SmsProvider();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SmsProvider.prototype).filter(m => m !== 'constructor');
