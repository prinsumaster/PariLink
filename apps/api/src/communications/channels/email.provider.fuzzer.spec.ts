...
describe('EmailProvider (Direct Fuzzer Suite)', () => {
  let service: EmailProvider;

  beforeEach(() => {
    try {
      service = new EmailProvider();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new EmailProvider();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(EmailProvider.prototype).filter(m => m !== 'constructor');
