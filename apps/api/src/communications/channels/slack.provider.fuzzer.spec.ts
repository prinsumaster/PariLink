...
describe('SlackProvider (Direct Fuzzer Suite)', () => {
  let service: SlackProvider;

  beforeEach(() => {
    try {
      service = new SlackProvider();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SlackProvider();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SlackProvider.prototype).filter(m => m !== 'constructor');
