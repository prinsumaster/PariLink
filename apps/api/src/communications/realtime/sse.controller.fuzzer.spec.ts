...
describe('SseController (Direct Fuzzer Suite)', () => {
  let service: SseController;

  beforeEach(() => {
    try {
      service = new SseController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SseController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SseController.prototype).filter(m => m !== 'constructor');
