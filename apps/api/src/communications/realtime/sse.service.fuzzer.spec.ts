...
describe('SseService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SseService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SseService.prototype).filter(m => m !== 'constructor');
  ...
  describe('subscribe (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.subscribe(...args as any);
        } catch (e) {
  ...
  describe('emitToUser (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.emitToUser(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('emitToCompany (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.emitToCompany(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
