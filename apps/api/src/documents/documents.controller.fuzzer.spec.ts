...
describe('DocumentsController (Direct Fuzzer Suite)', () => {
  let service: DocumentsController;

  beforeEach(() => {
    try {
      service = new DocumentsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DocumentsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DocumentsController.prototype).filter(m => m !== 'constructor');
