...
describe('DocumentFolderService (Direct Fuzzer Suite)', () => {
  let service: DocumentFolderService;

  beforeEach(() => {
    try {
      service = new DocumentFolderService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DocumentFolderService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DocumentFolderService.prototype).filter(m => m !== 'constructor');
