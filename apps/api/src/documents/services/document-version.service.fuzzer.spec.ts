...
describe('DocumentVersionService (Direct Fuzzer Suite)', () => {
  let service: DocumentVersionService;

  beforeEach(() => {
    try {
      service = new DocumentVersionService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DocumentVersionService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DocumentVersionService.prototype).filter(m => m !== 'constructor');
  ...
  describe('checkout (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.checkout(...args as any);
        } catch (e) {
  ...
  describe('checkin (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.checkin(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('unlock (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.unlock(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('revertToVersion (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined, undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.revertToVersion(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
