...
describe('StorageController (Direct Fuzzer Suite)', () => {
  let service: StorageController;

  beforeEach(() => {
    try {
      service = new StorageController();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new StorageController();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(StorageController.prototype).filter(m => m !== 'constructor');
  ...
  describe('downloadFile (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.downloadFile(...args as any);
        } catch (e) {
