...
describe('PaymentsService (Direct Fuzzer Suite)', () => {
  let service: PaymentsService;

  beforeEach(() => {
    try {
      service = new PaymentsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PaymentsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PaymentsService.prototype).filter(m => m !== 'constructor');
  ...
  describe('getPayments (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getPayments(...args as any);
        } catch (e) {
  ...
  describe('getPaymentsByInvoice (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.getPaymentsByInvoice(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
