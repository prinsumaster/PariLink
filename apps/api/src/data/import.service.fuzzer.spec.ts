...
describe('ImportService (Direct Fuzzer Suite)', () => {
  let service: ImportService;

  beforeEach(() => {
    try {
      service = new ImportService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ImportService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ImportService.prototype).filter(m => m !== 'constructor');
