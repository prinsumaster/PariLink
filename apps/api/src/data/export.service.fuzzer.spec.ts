...
describe('ExportService (Direct Fuzzer Suite)', () => {
  let service: ExportService;

  beforeEach(() => {
    try {
      service = new ExportService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ExportService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ExportService.prototype).filter(m => m !== 'constructor');
  ...
  describe('exportCompanyData (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.exportCompanyData(...args as any);
        } catch (e) {
