...
describe('DataController (Direct Fuzzer Suite)', () => {
  let service: DataController;

  beforeEach(() => {
    try {
      service = new DataController(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DataController(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DataController.prototype).filter(m => m !== 'constructor');
  ...
  describe('exportData (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.exportData(...args as any);
        } catch (e) {
  ...
  describe('importData (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.importData(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
