import { GstRuleController } from './gst-rule.controller';
...
describe('GstRuleController (Direct Fuzzer Suite)', () => {
  let service: GstRuleController;

  beforeEach(() => {
    try {
      service = new GstRuleController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new GstRuleController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(GstRuleController.prototype).filter(m => m !== 'constructor');
