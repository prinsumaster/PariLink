...
describe('GstEngineService (Direct Fuzzer Suite)', () => {
  let service: GstEngineService;

  beforeEach(() => {
    try {
      service = new GstEngineService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new GstEngineService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(GstEngineService.prototype).filter(m => m !== 'constructor');
