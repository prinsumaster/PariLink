...
describe('TrackingController (Direct Fuzzer Suite)', () => {
  let service: TrackingController;

  beforeEach(() => {
    try {
      service = new TrackingController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TrackingController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TrackingController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getLiveTracking (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getLiveTracking(...args as any);
        } catch (e) {
