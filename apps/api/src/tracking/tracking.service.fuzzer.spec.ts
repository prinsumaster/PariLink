...
describe('TrackingService (Direct Fuzzer Suite)', () => {
  let service: TrackingService;

  beforeEach(() => {
    try {
      service = new TrackingService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TrackingService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TrackingService.prototype).filter(m => m !== 'constructor');
