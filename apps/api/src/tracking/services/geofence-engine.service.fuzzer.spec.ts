...
describe('GeofenceEngineService (Direct Fuzzer Suite)', () => {
  let service: GeofenceEngineService;

  beforeEach(() => {
    try {
      service = new GeofenceEngineService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new GeofenceEngineService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(GeofenceEngineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('evaluateLocationAgainstGeofences (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
      ...
      for (const args of argSets) {
        try {
          await service.evaluateLocationAgainstGeofences(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
