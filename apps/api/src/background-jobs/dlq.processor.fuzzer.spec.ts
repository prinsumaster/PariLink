import { BackgroundJobsDlqProcessor } from './dlq.processor';
...
describe('BackgroundJobsDlqProcessor (Direct Fuzzer Suite)', () => {
  let service: BackgroundJobsDlqProcessor;

  beforeEach(() => {
    try {
      service = new BackgroundJobsDlqProcessor();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BackgroundJobsDlqProcessor();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BackgroundJobsDlqProcessor.prototype).filter(m => m !== 'constructor');
