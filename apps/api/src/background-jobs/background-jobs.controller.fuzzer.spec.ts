...
describe('BackgroundJobsController (Direct Fuzzer Suite)', () => {
  let service: BackgroundJobsController;

  beforeEach(() => {
    try {
      service = new BackgroundJobsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BackgroundJobsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BackgroundJobsController.prototype).filter(m => m !== 'constructor');
