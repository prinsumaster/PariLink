...
describe('AbacGuard (Direct Fuzzer Suite)', () => {
  let service: AbacGuard;

  beforeEach(() => {
    try {
      service = new AbacGuard(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AbacGuard(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AbacGuard.prototype).filter(m => m !== 'constructor');
