...
describe('ScopesGuard (Direct Fuzzer Suite)', () => {
  let service: ScopesGuard;

  beforeEach(() => {
    try {
      service = new ScopesGuard(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ScopesGuard(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ScopesGuard.prototype).filter(m => m !== 'constructor');
