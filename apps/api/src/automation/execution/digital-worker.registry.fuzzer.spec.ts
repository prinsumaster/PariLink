...
describe('DigitalWorkerRegistry (Direct Fuzzer Suite)', () => {
  let service: DigitalWorkerRegistry;

  beforeEach(() => {
    try {
      service = new DigitalWorkerRegistry();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DigitalWorkerRegistry();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DigitalWorkerRegistry.prototype).filter(m => m !== 'constructor');
  ...
  describe('registerWorker (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.registerWorker(...args as any);
        } catch (e) {
  ...
  describe('getWorker (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getWorker(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
