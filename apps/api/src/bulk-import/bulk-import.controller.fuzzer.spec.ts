...
describe('BulkImportController (Direct Fuzzer Suite)', () => {
  let service: BulkImportController;

  beforeEach(() => {
    try {
      service = new BulkImportController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BulkImportController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BulkImportController.prototype).filter(m => m !== 'constructor');
  ...
  describe('downloadTemplate (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.downloadTemplate(...args as any);
        } catch (e) {
  ...
  describe('importFile (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.importFile(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
