...
describe('UniversalSearchController (Direct Fuzzer Suite)', () => {
  let service: UniversalSearchController;

  beforeEach(() => {
    try {
      service = new UniversalSearchController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new UniversalSearchController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(UniversalSearchController.prototype).filter(m => m !== 'constructor');
  ...
  describe('globalSearch (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.globalSearch(...args as any);
        } catch (e) {
