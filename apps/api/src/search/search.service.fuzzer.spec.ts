...
describe('SearchService (Direct Fuzzer Suite)', () => {
  let service: SearchService;

  beforeEach(() => {
    try {
      service = new SearchService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SearchService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SearchService.prototype).filter(m => m !== 'constructor');
