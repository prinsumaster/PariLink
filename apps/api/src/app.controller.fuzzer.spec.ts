...
describe('AppController (Direct Fuzzer Suite)', () => {
  let service: AppController;

  beforeEach(() => {
    try {
      service = new AppController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new AppController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(AppController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getHello (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getHello(...args as any);
        } catch (e) {
