...
describe('LoadsService (Direct Fuzzer Suite)', () => {
  let service: LoadsService;

  beforeEach(() => {
    try {
      service = new LoadsService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LoadsService(null, null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LoadsService.prototype).filter(m => m !== 'constructor');
