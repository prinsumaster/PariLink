...
describe('LoadsController (Direct Fuzzer Suite)', () => {
  let service: LoadsController;

  beforeEach(() => {
    try {
      service = new LoadsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LoadsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LoadsController.prototype).filter(m => m !== 'constructor');
