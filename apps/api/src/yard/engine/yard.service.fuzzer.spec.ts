...
describe('YardService (Direct Fuzzer Suite)', () => {
  let service: YardService;

  beforeEach(() => {
    try {
      service = new YardService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new YardService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(YardService.prototype).filter(m => m !== 'constructor');
