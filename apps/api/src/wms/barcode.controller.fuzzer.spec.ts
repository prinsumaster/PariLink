...
describe('BarcodeController (Direct Fuzzer Suite)', () => {
  let service: BarcodeController;

  beforeEach(() => {
    try {
      service = new BarcodeController();
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BarcodeController();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BarcodeController.prototype).filter(m => m !== 'constructor');
  ...
  describe('processScan (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.processScan(...args as any);
        } catch (e) {
