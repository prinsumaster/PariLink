...
describe('WorkflowExecutorService (Direct Fuzzer Suite)', () => {
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WorkflowExecutorService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WorkflowExecutorService.prototype).filter(m => m !== 'constructor');
