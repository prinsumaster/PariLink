...
describe('ActionEngineService (Direct Fuzzer Suite)', () => {
  let service: ActionEngineService;

  beforeEach(() => {
    try {
      service = new ActionEngineService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ActionEngineService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(ActionEngineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('executeAction (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.executeAction(...args as any);
        } catch (e) {
