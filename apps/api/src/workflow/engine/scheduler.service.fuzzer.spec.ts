...
describe('SchedulerEngineService (Direct Fuzzer Suite)', () => {
  let service: SchedulerEngineService;

  beforeEach(() => {
    try {
      service = new SchedulerEngineService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new SchedulerEngineService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(SchedulerEngineService.prototype).filter(m => m !== 'constructor');
  ...
  describe('scheduleDelay (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.scheduleDelay(...args as any);
        } catch (e) {
