...
describe('WorkflowExecutionProcessor (Direct Fuzzer Suite)', () => {
  let service: WorkflowExecutionProcessor;

  beforeEach(() => {
    try {
      service = new WorkflowExecutionProcessor(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WorkflowExecutionProcessor(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WorkflowExecutionProcessor.prototype).filter(m => m !== 'constructor');
