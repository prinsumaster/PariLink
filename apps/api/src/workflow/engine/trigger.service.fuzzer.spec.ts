import { TriggerEngineService } from './trigger.service';
...
describe('TriggerEngineService (Direct Fuzzer Suite)', () => {
  let service: TriggerEngineService;

  beforeEach(() => {
    try {
      service = new TriggerEngineService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TriggerEngineService(null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TriggerEngineService.prototype).filter(m => m !== 'constructor');
