...
describe('CustomersService (Direct Fuzzer Suite)', () => {
  let service: CustomersService;

  beforeEach(() => {
    try {
      service = new CustomersService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CustomersService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CustomersService.prototype).filter(m => m !== 'constructor');
