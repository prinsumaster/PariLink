...
describe('CustomersController (Direct Fuzzer Suite)', () => {
  let service: CustomersController;

  beforeEach(() => {
    try {
      service = new CustomersController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new CustomersController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(CustomersController.prototype).filter(m => m !== 'constructor');
