...
describe('BranchesController (Direct Fuzzer Suite)', () => {
  let service: BranchesController;

  beforeEach(() => {
    try {
      service = new BranchesController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BranchesController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BranchesController.prototype).filter(m => m !== 'constructor');
