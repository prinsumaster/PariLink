...
describe('BranchesService (Direct Fuzzer Suite)', () => {
  let service: BranchesService;

  beforeEach(() => {
    try {
      service = new BranchesService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BranchesService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BranchesService.prototype).filter(m => m !== 'constructor');
