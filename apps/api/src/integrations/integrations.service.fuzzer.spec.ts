...
describe('IntegrationsService (Direct Fuzzer Suite)', () => {
  let service: IntegrationsService;

  beforeEach(() => {
    try {
      service = new IntegrationsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new IntegrationsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(IntegrationsService.prototype).filter(m => m !== 'constructor');
