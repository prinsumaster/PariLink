...
describe('TwilioService (Direct Fuzzer Suite)', () => {
  let service: TwilioService;

  beforeEach(() => {
    try {
      service = new TwilioService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new TwilioService(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(TwilioService.prototype).filter(m => m !== 'constructor');
  ...
  describe('sendWhatsAppMessage (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.sendWhatsAppMessage(...args as any);
        } catch (e) {
