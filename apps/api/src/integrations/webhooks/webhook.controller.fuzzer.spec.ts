...
describe('WebhookController (Direct Fuzzer Suite)', () => {
  let service: WebhookController;

  beforeEach(() => {
    try {
      service = new WebhookController(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new WebhookController(null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(WebhookController.prototype).filter(m => m !== 'constructor');
