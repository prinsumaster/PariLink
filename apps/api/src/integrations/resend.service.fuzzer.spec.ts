...
describe('ResendService (Direct Fuzzer Suite)', () => {
  let service: ResendService;

  beforeEach(() => {
    try {
      service = new ResendService(makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new ResendService(null, null);
    } catch(e) {}
  ...
  describe('sendTransactionalEmail (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.sendTransactionalEmail(...args as any);
        } catch (e) {
