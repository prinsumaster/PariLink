...
describe('LocoNavService (Direct Fuzzer Suite)', () => {
  let service: LocoNavService;
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LocoNavService();
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LocoNavService.prototype).filter(m => m !== 'constructor');
  ...
  describe('trackVehicle (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.trackVehicle(...args as any);
        } catch (e) {
  ...
  describe('getCurrentLocation (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.getCurrentLocation(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
