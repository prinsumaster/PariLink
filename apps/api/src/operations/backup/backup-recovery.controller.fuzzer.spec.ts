...
describe('BackupRecoveryController (Direct Fuzzer Suite)', () => {
  let service: BackupRecoveryController;

  beforeEach(() => {
    try {
      service = new BackupRecoveryController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new BackupRecoveryController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(BackupRecoveryController.prototype).filter(m => m !== 'constructor');
  ...
  describe('startBackup (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.startBackup(...args as any);
        } catch (e) {
  ...
  describe('verifyBackup (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.verifyBackup(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('restore (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.restore(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('purge (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.purge(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
