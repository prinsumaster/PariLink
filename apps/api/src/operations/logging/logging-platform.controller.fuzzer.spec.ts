import { LoggingPlatformController } from './logging-platform.controller';
...
describe('LoggingPlatformController (Direct Fuzzer Suite)', () => {
  let service: LoggingPlatformController;

  beforeEach(() => {
    try {
      service = new LoggingPlatformController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new LoggingPlatformController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(LoggingPlatformController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getErrorGroups (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined],
      ...
      for (const args of argSets) {
        try {
          await service.getErrorGroups(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
