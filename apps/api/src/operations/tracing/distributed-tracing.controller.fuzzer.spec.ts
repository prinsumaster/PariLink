import { DistributedTracingController } from './distributed-tracing.controller';
...
describe('DistributedTracingController (Direct Fuzzer Suite)', () => {
  let service: DistributedTracingController;

  beforeEach(() => {
    try {
      service = new DistributedTracingController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new DistributedTracingController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(DistributedTracingController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getTrace (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getTrace(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
  ...
  describe('getSlowestSpans (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      if (!service) return;
      const argSets = [
          [undefined, undefined],
      ...
      for (const args of argSets) {
        try {
          await service.getSlowestSpans(...args as any);
        } catch (e) {
          // ignore error for fuzzing
        }
      }
