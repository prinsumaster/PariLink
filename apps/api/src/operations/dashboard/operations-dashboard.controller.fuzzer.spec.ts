...
describe('OperationsDashboardController (Direct Fuzzer Suite)', () => {
  let service: OperationsDashboardController;

  beforeEach(() => {
    try {
      service = new OperationsDashboardController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new OperationsDashboardController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(OperationsDashboardController.prototype).filter(m => m !== 'constructor');
  ...
  describe('getSnapshot (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getSnapshot(...args as any);
        } catch (e) {
