...
describe('OperationsDashboardService (Direct Fuzzer Suite)', () => {
  let service: OperationsDashboardService;

  beforeEach(() => {
    try {
      service = new OperationsDashboardService(makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'), makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new OperationsDashboardService(null, null, null, null, null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(OperationsDashboardService.prototype).filter(m => m !== 'constructor');
  ...
  describe('getDashboardSnapshot (Fuzzer)', () => {
    it('should handle fuzzing gracefully', async () => {
      ...
      for (const args of argSets) {
        try {
          await service.getDashboardSnapshot(...args as any);
        } catch (e) {
