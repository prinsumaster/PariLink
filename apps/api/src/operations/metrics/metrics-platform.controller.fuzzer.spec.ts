import { MetricsPlatformController } from './metrics-platform.controller';
...
describe('MetricsPlatformController (Direct Fuzzer Suite)', () => {
  let service: MetricsPlatformController;

  beforeEach(() => {
    try {
      service = new MetricsPlatformController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new MetricsPlatformController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(MetricsPlatformController.prototype).filter(m => m !== 'constructor');
