...
describe('VendorsController (Direct Fuzzer Suite)', () => {
  let service: VendorsController;

  beforeEach(() => {
    try {
      service = new VendorsController(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new VendorsController(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(VendorsController.prototype).filter(m => m !== 'constructor');
