...
describe('PurchaseOrderService (Direct Fuzzer Suite)', () => {
  let service: PurchaseOrderService;

  beforeEach(() => {
    try {
      service = new PurchaseOrderService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new PurchaseOrderService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(PurchaseOrderService.prototype).filter(m => m !== 'constructor');
