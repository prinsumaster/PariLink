...
describe('VendorsService (Direct Fuzzer Suite)', () => {
  let service: VendorsService;

  beforeEach(() => {
    try {
      service = new VendorsService(makeDeepProxy('dep'));
    } catch(e) {}
  });
  ...
  it('should hit branches with null deps', async () => {
    let nullService;
    try {
      nullService = new VendorsService(null);
    } catch(e) {}
    
    if (nullService) {
      const methods = Object.getOwnPropertyNames(VendorsService.prototype).filter(m => m !== 'constructor');
