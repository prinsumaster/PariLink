...
import { WMSFilters as FilterState } from '@/types/wms';
import Link from 'next/link';
import { Plus, Download } from 'lucide-react';
import { Button } from '@/components/ui/button';

import { WarehouseTable } from '@/components/wms/warehouse-table';
import { WarehouseFilters } from '@/components/wms/warehouse-filters';
...
export default function WMSPage() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['wms', filters],
    queryFn: () => wmsService.getWarehouses(filters),
  });
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">Warehouse Network</h1>
          <p className="text-base font-medium text-slate-500 dark:text-slate-400 mt-1">Manage distribution centers, capacity, and zones.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <RoleGuard allowedRoles={['SUPER_ADMIN', 'ORG_ADMIN', 'OPERATIONS']} fallback={null}>
            <Link href="/wms/new" passHref>
