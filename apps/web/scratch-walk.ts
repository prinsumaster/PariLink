...
async function main() {
  ...
  for (const r of routes) {
    ...
    await page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {}); // ignore timeout if animations keep it active
    ...
    const rowCount = await page.evaluate(() => {
      ...
      const cards = document.querySelectorAll('[class*="border"][class*="rounded-xl"], [class*="bg-slate-900"]');
      return cards.length;
    });
